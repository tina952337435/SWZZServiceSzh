package swzzmodeserver.workserver.data.swzzdata;

import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;
import swzzmodeserver.workserver.pojo.swzzdata.Qx_keyPojo;
import swzzmodeserver.workserver.pojo.swzzdata.Qx_mainPojo;

import java.util.List;

@Mapper
public interface Qx_mainData {

    List<Qx_mainPojo> selectList(
                                 @Param(value = "ID") String ID,
                                 @Param(value = "key") String key,
                                 @Param(value = "stime") String stime,
                                 @Param(value = "etime") String etime,
                                 @Param(value = "type")String type,
                                 @Param(value = "QX_SYS") String QX_SYS,
                                 @Param(value = "startindex") Integer startindex,
                                 @Param(value = "pagesize") Integer pagesize);

    Integer updateOne(Qx_mainPojo bdmsPredictPojo);

    Integer insertOne(Qx_mainPojo bdmsPredictPojo);

    Integer deleteOne(@Param(value = "ID") String ID);

    Integer selectCount(
                        @Param(value = "ID") String ID,
                        @Param(value = "key") String key,
                        @Param(value = "stime") String stime,
                        @Param(value = "etime") String etime,
                        @Param(value = "type")String type,
                        @Param(value = "QX_SYS") String QX_SYS);
}
