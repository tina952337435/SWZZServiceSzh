...
public class NmcTyphoonService {
    ...
    public TyphoonSummary findTyphoonById(List<TyphoonSummary> list, String id) {
        ...
        for (TyphoonSummary ts : list) {
            if (id.equals(String.valueOf(ts.getId()))) return ts;
        }
        return null;
    }
    ...
    public TyphoonSummary findTyphoon(int year, String code, String name) throws Exception {
        ...
        for (TyphoonSummary ts : list) {
            if (name != null && !name.isEmpty() && name.equals(ts.getCnName())) return ts;
            if (code != null && !code.isEmpty()
                    && (code.equals(ts.getCode1()) || code.equals(ts.getCode2()))) return ts;
        }
        return null;
    }
    ...
    public TyphoonDetail getTyphoonDetail(int typhoonId) throws Exception {
        ...
        if (typhoon == null || !typhoon.isArray()) return null;
    ...
    public TrackPoint getLatestObservation(TyphoonDetail detail) {
        if (detail == null || detail.getTracks() == null || detail.getTracks().isEmpty()) return null;
        return detail.getTracks().get(detail.getTracks().size() - 1);
    }

    public List<ForecastPoint> getCmaForecast(TyphoonDetail detail) {
        if (detail == null || detail.getTracks() == null) return new ArrayList<>();
        ...
        if (latest != null && latest.getForecasts() != null) {
            ...
            for (String agency : new String[]{"BABJ", "BCC"}) {
                if (forecasts.containsKey(agency)) return forecasts.get(agency);
            }
            if (!forecasts.isEmpty()) return forecasts.values().iterator().next();
        }
    ...
    public String fetchTrackImageUrl(String typhoonCode) {
        if (typhoonCode == null) return null;
        try {
    ...
    public String downloadTrackImage(String imageUrl, String saveDir, String typhoonCode) {
        ...
        try {
            ...
            org.apache.http.client.methods.HttpGet get =
                    new org.apache.http.client.methods.HttpGet(imageUrl);
    ...
    public String buildTyphoonStatusText(TyphoonDetail detail, TrackPoint obs,
        ...
        // 观测时间: "7月8日8时"
        sb.append(formatObsTime(obs.getLocalTimeChinese()));
    ...
    private String buildForecastText(TyphoonDetail detail, TrackPoint obs,
        ...
        if (!forecast.isEmpty()) {
            ...
            if (Math.abs(latDiff) > 2 || Math.abs(lonDiff) > 2) {
                String dirDesc = "";
                if (latDiff > 1 && lonDiff < -2) dirDesc = "向西北方向移动";
                else if (latDiff > 1 && Math.abs(lonDiff) < 2) dirDesc = "向北移动";
                else if (Math.abs(latDiff) < 1 && lonDiff < -2) dirDesc = "向西移动";
                else if (latDiff > 1 && lonDiff < -1) dirDesc = "向偏西方向移动";
    ...
    private String gradeToFullChinese(String grade) {
        ...
        switch (grade) {
            case "TD": return "热带低压";
            case "TS": return "";
            case "STS": return "强热带风暴级";
            case "TY": return "";
            case "STY": return "强";
            case "SuperTY": return "超强";
            default: return "";
        }
    }
    ...
    private String windToGrade(int windMs) {
        if (windMs >= 61) return "17级以上";
        if (windMs >= 51) return "16级";
        if (windMs >= 46) return "15级";
        if (windMs >= 41) return "14级";
        if (windMs >= 37) return "13级";
        if (windMs >= 33) return "12级";
        if (windMs >= 29) return "11级";
        if (windMs >= 25) return "10级";
        if (windMs >= 21) return "9级";
        if (windMs >= 17) return "8级";
        if (windMs >= 14) return "7级";
        return windMs + "米/秒";
    }
    ...
    private String dirToChinese(String dir) {
        ...
        switch (dir.toUpperCase()) {
            case "N": return "北";
            case "NNE": return "东北偏北";
            case "NE": return "东北";
            case "ENE": return "东北偏东";
            case "E": return "东";
            case "ESE": return "东南偏东";
            case "SE": return "东南";
            case "SSE": return "东南偏南";
            case "S": return "南";
    ...
    private String computeIntensityTrend(TrackPoint obs, List<ForecastPoint> forecast) {
        ...
        if (windDiff >= 5) return "强度明显加强";
        if (windDiff >= 2) return "强度有所加强";
        if (windDiff <= -5 || pressDiff >= 20) return "强度逐渐减弱";
        if (windDiff <= -2) return "强度略有下降";
        return "强度变化不大";
    }

    /** 从风圈列表中提取指定级别的风圈 [kts, NE, SE, SW, NW] */
    private int[] getWindRadiiByKts(TrackPoint obs, int kts) {
        if (obs.getWindRadii() == null) return null;
    ...
    private String httpGet(String urlStr) throws Exception {
        ...
        org.apache.http.client.methods.HttpGet get =
                new org.apache.http.client.methods.HttpGet(urlStr);
    ...
    private JsonNode parseJsonp(String jsonp) throws Exception {
        ...
        if (start >= 0 && end > start) return mapper.readTree(jsonp.substring(start + 1, end));
        return mapper.readTree(jsonp);
    }

    private TrackPoint parseTrackPoint(JsonNode node) {
        if (node == null || !node.isArray() || node.size() < 10) return null;
        ...
        if (node.size() > 10 && node.get(10).isArray()) {
            ...
            for (JsonNode rNode : node.get(10)) {
                if (rNode.isArray() && rNode.size() >= 5) {
                    ...
                    int kts = ktsLabel.contains("30") ? 30 : ktsLabel.contains("50") ? 50
    ...
    private static int min(int a, int b, int c, int d) { return Math.min(Math.min(a, b), Math.min(c, d)); }
    private static int max(int a, int b, int c, int d) { return Math.max(Math.max(a, b), Math.max(c, d)); }
    ...
    public static class TyphoonSummary {
        ...
        public void setEnName(String v) { enName = v; }
        public String getCnName() { return cnName; }
        public void setCnName(String v) { cnName = v; }
        public String getCode1() { return code1; }
        public void setCode1(String v) { code1 = v; }
        public String getCode2() { return code2; }
        public void setCode2(String v) { code2 = v; }
        public String getCmaNum() { return cmaNum; }
        public void setCmaNum(String v) { cmaNum = v; }
    ...
    public static class TyphoonDetail {
        ...
        public String getStatus() { return status; }
        public void setStatus(String v) { status = v; }
        public List<TrackPoint> getTracks() { return tracks; }
        public void setTracks(List<TrackPoint> v) { tracks = v; }
    }
    ...
    public static class TrackPoint {
        ...
        public void setUtcTime(String v) { utcTime = v; }
        ...
        public void setWindSpeed(int v) { windSpeed = v; }
        public String getDirection() { return direction; }
        public void setDirection(String v) { direction = v; }
        public int getSpeed() { return speed; }
        public void setSpeed(int v) { speed = v; }
        public List<int[]> getWindRadii() { return windRadii; }
        public void setWindRadii(List<int[]> v) { windRadii = v; }
        public Map<String, List<ForecastPoint>> getForecasts() { return forecasts; }
        public void setForecasts(Map<String, List<ForecastPoint>> v) { forecasts = v; }
        public String getLocalTime() { return localTime; }
        public void setLocalTime(String v) { localTime = v; }
        public String getLocalTimeChinese() { return localTimeChinese; }
        public void setLocalTimeChinese(String v) { localTimeChinese = v; }
        public String getDescription() { return description; }
        public void setDescription(String v) { description = v; }
    }
    ...
    public static class ForecastPoint {
        ...
        public void setAgency(String v) { agency = v; }
        public String getGrade() { return grade; }
        public void setGrade(String v) { grade = v; }
    }
}
