...
public class ST_GATE_RNEWController {
    ...
    public List<ST_STBPRP_FCCHPojo> selectGQList(@RequestBody ColumnName param) {
        ...
        String key = "", stime = DateUtil.dateFormat(new Date(), "yyyy-MM-dd HH:mm:ss"), etime = "", PID = "",
                stcd = "";
        ...
        for (ST_STBPRP_FCCHPojo stStbprpFcchPojo : sfList) {
            ...
            List<ST_GATE_RNEWPojo> collect = gateList.stream()
                    .filter(i -> i.getSTCD().equals(stStbprpFcchPojo.getSTCD()) // && ("".equals(finalKey) ||
                                                                                // finalKey.equals(i.getEXKEY()))
                    ).collect(Collectors.toList());
            stStbprpFcchPojo.setGateList(collect);
            List<GetWaterViewNewPojo> collects = waterViewNewList.stream()
        ...
        for (ST_STBPRP_FCCHPojo stStbprpFcchPojo : sfList) {
            ...
            List<ST_GATE_RPojo> collect = gateList.stream().filter(i -> {
                return i.getSTCD().equals(stStbprpFcchPojo.getSTCD())
                        && ("".equals(finalKey) || finalKey.equals(i.getEXKEY()));
            }).collect(Collectors.toList());
    ...
    public ResultUtils queryList(@RequestBody ColumnName param) {
        ...
        if (null != param.getStcd()) {
            ...
            quList = quData.queryList(null, null, null, pidList);
            for (ST_STBPRP_B_QUPojo quPojo : quList) {
                if (null != quPojo.getSTCD()) {
                    stcdList.add(quPojo.getSTCD());
                }
        ...
        for (ST_STBPRP_FCCHPojo stStbprpFcchPojo : sfList) {
            ...
            List<ST_STBPRP_B_QUPojo> onequstcd = quList.stream()
                    ...
                    .collect(Collectors.toList());
            if (onequstcd.size() > 0) {
                stStbprpFcchPojo.setDIR(onequstcd.get(0).getDIR());
                stStbprpFcchPojo.setMAPSIZE(String.valueOf(onequstcd.get(0).getMAPSIZE()));
                stStbprpFcchPojo.setROTATE(onequstcd.get(0).getROTATE());
            }
            ...
            if (treePojoList.size() > 0) {
                treePojoList.forEach(u -> {
                    ...
                    if (null != quList && quList.size() > 0) {
                        List<String> stcdList = quList.stream().map(item -> item.getSTCD()).distinct()
                                .collect(Collectors.toList());
                        List<ST_GATE_RNEWPojo> gateList = data.selectGateListNew(stcdList);
                        AtomicReference<Integer> kaiCount = new AtomicReference<>(0);
                        quList.forEach(stStbprpFcchPojo -> {
                            List<ST_GATE_RNEWPojo> collect = gateList.stream()
                                    .filter(i -> i.getSTCD().equals(stStbprpFcchPojo.getSTCD()))
                                    .collect(Collectors.toList());
                            List<ST_GATE_RNEWPojo> collectKai = collect.stream().filter(i -> i.getGTOPHGT() >= 0.1)
                                    .collect(toList());// 开启的闸和泵
