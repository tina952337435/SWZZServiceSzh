...
public class ST_PPTN_RController {
    ...
    public ResultUtils selectListIsDay(@RequestBody ColumnName param){
        ...
        String stcd="",stime= DateUtil.dateFormat(date,"yyyy-MM-dd HH:mm:ss"),etime="",PID="",ADMAUTH="",STNM="";
        ...
        if(stcdList.size() > 0){
            for(String  item : stcdList){
                List<ST_STBPRP_BPojo> collect = stStbprpBPList.stream().filter(i -> i.getSTCD().equals(item)).collect(toList());
                if(collect.size() > 0){
                    ...
                    List<ST_PPTN_RPojo> drpList = sprlist.stream().filter(i -> i.getSTCD().equals(item)).collect(toList());
                    if(drpList.size() > 0) {
    ...
    public ResultUtils selectList(@RequestBody ColumnName param){
        ...
        if(null != quList){
            for(ST_STBPRP_B_QUPojo quPojo : quList){
                ...
                List<ST_PPTN_RPojo> collect = pptnList.stream().filter(i -> i.getSTCD().equals(quDto.getSTCD())).collect(toList());
    ...
    public ResultUtils selectListByHouseMax(@RequestBody ColumnName param){
        ...
        if(null != quList && quList.size() > 0){
            ...
            for(ST_STBPRP_B_QUPojo quPojo : quList){
                ST_STBPRP_B_QUDto quDto = new ST_STBPRP_B_QUDto();
                BeanUtils.copyProperties(quPojo,quDto);
                List<ST_PPTN_RPojo> collect = sprlist.stream().filter(i -> i.getSTCD().equals(quDto.getSTCD())).collect(toList());
                if(collect.size() > 0){
...
//            );
            ...
            for(ST_STBPRP_B_TREEPojo xzpojo : quList){
                List<ST_STBPRP_B_QUPojo> quPojos = quPojosAll.stream().filter(u->u.getPID().equals(xzpojo.getID())).collect(toList());
...
//                List<ST_PPTN_RPojo> pptnPojos = data.selectListIsDay(stcds, stime, etime);
                List<ST_PPTN_RPojo> pptnPojos = pptnPojosAll.stream().filter(u->stcds.contains(u.getSTCD())).collect(toList());//data.selectListIsDay(stcds, stime, etime);
    ...
    public ResultUtils selectListByTime(@RequestBody ColumnName param){
        ...
        if(null != quList && quList.size() > 0){
            ...
            for(ST_STBPRP_B_QUDto quPojo : quList){
                List<ST_PPTN_RPojo> collect = sprlist.stream().filter(i -> i.getSTCD().equals(quPojo.getSTCD())).collect(toList());
...
//            }
        ...
        if(null != quList && quList.size() > 0){
            ...
            for(ST_STBPRP_BPojo rpBPojo : stbbList){
                ...
                List<ST_PPTN_RDto> collect = sprlist.stream().filter(i -> i.getSTCD().equals(rpBDto.getSTCD())).collect(toList());
                if(collect.size() > 0){
...
//    @ResponseBody
//    public List<Map<String,Object>> dengzhixian(@RequestBody DENGZHIXIANYQPojo param){
//        StopWatch watch = new StopWatch();
//        // 计时器开始
//        watch.start();
...
//    @ResponseBody
//    public ResultUtils<List> dengzhixianNew(@RequestBody DENGZHIXIANYQPojo param){
//        StopWatch watch = new StopWatch();
//        // 计时器开始
//        watch.start();
//        List<String> listSTCD= new ArrayList<>();
...
//        watch.stop();
//
//        return new ResultUtils<>(list, "操作成功", true, 1, 1, 0, 0, watch.getTime(), null);
//    }
...
//            pptnList = data.selectListDanZhanByHouse(stcdList,stime,etime);
        ...
        for(int i = 0;i < timedif;i++){
            ...
            List<ST_PPTN_RPojo> collect = pptnList.stream().filter(n -> null != n.getDRP() && n.getDRP() > 0).collect(toList());
            ...
            //计算日最大降雨站点
            List<ST_PPTN_RPojo> mRainListMax = collectList.stream().filter(u -> null != u.getDRP() && u.getDRP() > 0.1).sorted(Comparator.comparing(ST_PPTN_RPojo::getDRP).reversed()).collect(toList());
            ...
            if(null != mRainListMax && mRainListMax.size() > 0 && mRainListMax.get(0).getDRP() > 0.2){
                ...
                List<ST_STBPRP_BPojo> info = bPojos.stream().filter(k -> k.getSTCD().equals(mRainListMax.get(0).getSTCD())).collect(toList());
...
//                }
                ...
                if(null != pptnList && pptnList.size() > 0){
                    ...
                    for(ST_STBPRP_B_QUPojo quPojo : quPojos) {
                        // 对象根据降雨属性降序排序
                        List<ST_PPTN_RPojo> newList = pptnList.stream().filter(k -> k.getSTCD().equals(quPojo.getSTCD())).sorted(Comparator.comparing(ST_PPTN_RPojo::getDRP).reversed()).collect(toList());
...
//        List<ST_PPTN_RPojo> sprlist = data.queryDRPList(stime, etime,ADMAUTH,stcdList);
        ...
        if(pathname.equals("DAY")){
            ...
            if(dayCount>0){
                ...
                for(int num=0;num<=dayCount;num++){
                    Date time=DateUtil.addDays(DateUtil.strToDate(DateUtil.pastTM(stime,DateUtil.YMD)+" 08:00:00",DateUtil.YMDHMS),num);
                    Date finatime=DateUtil.addDays(time,1);
//                    List<ST_PPTN_RPojo> tempDrpList=sprlist.stream().filter(u->DateUtil.strToDate(u.getTM(),DateUtil.YMDHMS).getTime()>time.getTime()&&DateUtil.strToDate(u.getTM(),DateUtil.YMDHMS).getTime()<=finatime.getTime()).collect(toList());
                    String _stime=DateUtil.dateToStr(time,DateUtil.YMDHMS);
                    String _etime=DateUtil.dateToStr(finatime,DateUtil.YMDHMS);
                    List<ST_PPTN_RPojo> tempDrpList=sprlist.stream().filter(u->u.getTM().compareTo(_stime)>0&&u.getTM().compareTo(_etime)<=0).collect(toList());

                    if(tempDrpList.size()>0){
                        Map<String, List<ST_PPTN_RPojo>> mapDRP=tempDrpList.stream().collect(Collectors.groupingBy(ST_PPTN_RPojo::getSTCD));
...
//                    List<ST_PPTN_RPojo> tempDrpList=sprlist.stream().filter(u->DateUtil.strToDate(u.getTM(),DateUtil.YMDHMS).getTime()>time.getTime()&&DateUtil.strToDate(u.getTM(),DateUtil.YMDHMS).getTime()<=finatime.getTime()).collect(toList());
        ...
        **/
            ...
            /**
            Map<String, List<ST_PPTN_RPojo>> mapDRP = sprlist.stream().collect(Collectors.groupingBy(ST_PPTN_RPojo::getSTCD));
        ...
        if(!CommonUtills.isEmpty(PID)){
            ...
            if(treePojoList.size()>0){
                ...
                treePojoList.forEach(u->{
                    try {
                        //查询配置的站码
                        List<ST_STBPRP_B_QUPojo> quList = stBprpBquList.stream().filter(o->o.getPID().equals(u.getID())).collect(toList());
                        if(null != quList && quList.size() > 0){
                            List<String> stcdList = quList.stream().map(item -> item.getSTCD()).collect(Collectors.toList());
    ...
    public ResultUtils queryMAXHOURDRPList(@RequestBody ColumnName param){
        ...
        if(!CommonUtills.isEmpty(PID)){
            ...
            Map<String, List<ST_PPTN_RPojo>> mapDRP = oneuserList.stream().collect(Collectors.groupingBy(ST_PPTN_RPojo::getSTCD));
            mapDRP.forEach((key, value)->{
                List<ST_PPTN_RPojo> itemList=value.stream().sorted(Comparator.comparing(ST_PPTN_RPojo::getDRP).reversed()).collect(Collectors.toList());
                if(itemList.size()>0){
                    ST_PPTN_RPojo pojo=new ST_PPTN_RPojo();
    ...
    public ResultUtils querySingleStationRainAnalysis(@RequestBody ColumnName param){
        ...
        for(int i = 0;i < timedif;i++){
            ...
            List<ST_PPTN_RPojo> mListTemp = mList.stream().filter(j -> {
                if(null != j.getTM()){
                    try {
                        return dateFormat.parse(j.getTM()).after(final_stm) && dateFormat.parse(j.getTM()).before(final_etm);
                    } catch (ParseException e) {
                        e.printStackTrace();
                    }
                }
            ...
            //最高雨量
            Double maxZ = (new BigDecimal(mListTemp.get(0).getDRP()).setScale(1, BigDecimal.ROUND_HALF_UP)).doubleValue();
    ...
    public ResultUtils queryStationWaterRainAnalysis(@RequestBody ColumnName param){
        ...
        if(stStbprpBPList.size() > 0) {
            for (ST_STBPRP_BPojo item : stStbprpBPList) {
                ...
                String finalStime = stime;
                //水位
                List<GetWaterViewNewPojo> listTemp = list.stream().filter(p->DateUtil.strToDate(p.getTM(),DateUtil.YMDHMS).getTime()==DateUtil.strToDate(finalStime,DateUtil.YMDHMS).getTime()&&p.getSTCD().equals(_stcd)).collect(Collectors.toList());
                if(listTemp.size()>0){
                    Double strVal=Double.valueOf(listTemp.get(0).getUPZ());
                    pojo.setZ1(strVal);
                }
                listTemp = list.stream().filter(p->DateUtil.strToDate(p.getTM(),DateUtil.YMDHMS).getTime()==DateUtil.strToDate(ED3Str,DateUtil.YMDHMS).getTime()&&p.getSTCD().equals(_stcd)).collect(Collectors.toList());
                if(listTemp.size()>0){
                    Double strVal=Double.valueOf(listTemp.get(0).getUPZ());
                    pojo.setZ3(strVal);
                }
                listTemp = list.stream().filter(p->DateUtil.strToDate(p.getTM(),DateUtil.YMDHMS).getTime()==DateUtil.strToDate(ED6Str,DateUtil.YMDHMS).getTime()&&p.getSTCD().equals(_stcd)).collect(Collectors.toList());
                if(listTemp.size()>0){
                    Double strVal=Double.valueOf(listTemp.get(0).getUPZ());
                    pojo.setZ6(strVal);
                }
                listTemp = list.stream().filter(p->DateUtil.strToDate(p.getTM(),DateUtil.YMDHMS).getTime()==DateUtil.strToDate(ED9Str,DateUtil.YMDHMS).getTime()&&p.getSTCD().equals(_stcd)).collect(Collectors.toList());
                if(listTemp.size()>0){
                    Double strVal=Double.valueOf(listTemp.get(0).getUPZ());
                    pojo.setZ9(strVal);
                }
                listTemp = list.stream().filter(p->DateUtil.strToDate(p.getTM(),DateUtil.YMDHMS).getTime()==DateUtil.strToDate(ED12Str,DateUtil.YMDHMS).getTime()&&p.getSTCD().equals(_stcd)).collect(Collectors.toList());
                if(listTemp.size()>0){
                    Double strVal=Double.valueOf(listTemp.get(0).getUPZ());
                    pojo.setZ12(strVal);
                }
                listTemp = list.stream().filter(p->DateUtil.strToDate(p.getTM(),DateUtil.YMDHMS).getTime()==DateUtil.strToDate(ED24Str,DateUtil.YMDHMS).getTime()&&p.getSTCD().equals(_stcd)).collect(Collectors.toList());
                if(listTemp.size()>0){
                    Double strVal=Double.valueOf(listTemp.get(0).getUPZ());
                    pojo.setZ24(strVal);
                }
                ...
                List<ST_PPTN_RPojo> mListTemp = mList.stream().filter(j -> {
                    if(null != j.getTM()){
                        try {
                            return dateFormat.parse(j.getTM()).after(new Date(finalSTime1)) && dateFormat.parse(j.getTM()).before(ED3)&&j.getSTCD().equals(_stcd);
                        } catch (ParseException e) {
                            e.printStackTrace();
                        }
                    }
                ...
                mListTemp = mList.stream().filter(j -> {
                    if(null != j.getTM()){
                        try {
                            return dateFormat.parse(j.getTM()).after(new Date(finalSTime1)) && dateFormat.parse(j.getTM()).before(ED6)&&j.getSTCD().equals(_stcd);
                        } catch (ParseException e) {
                            e.printStackTrace();
                        }
                    }
                ...
                mListTemp = mList.stream().filter(j -> {
                    if(null != j.getTM()){
                        try {
                            return dateFormat.parse(j.getTM()).after(new Date(finalSTime1)) && dateFormat.parse(j.getTM()).before(ED9)&&j.getSTCD().equals(_stcd);
                        } catch (ParseException e) {
                            e.printStackTrace();
                        }
                    }
                ...
                mListTemp = mList.stream().filter(j -> {
                    if(null != j.getTM()){
                        try {
                            return dateFormat.parse(j.getTM()).after(new Date(finalSTime1)) && dateFormat.parse(j.getTM()).before(ED12)&&j.getSTCD().equals(_stcd);
                        } catch (ParseException e) {
                            e.printStackTrace();
                        }
                    }
                ...
                mListTemp = mList.stream().filter(j -> {
                    if(null != j.getTM()){
                        try {
                            return dateFormat.parse(j.getTM()).after(new Date(finalSTime1)) && dateFormat.parse(j.getTM()).before(ED24)&&j.getSTCD().equals(_stcd);
                        } catch (ParseException e) {
                            e.printStackTrace();
                        }
                    }
    ...
    public ResultUtils queryFloodSituationMessage(@RequestBody ColumnName param){
        ...
        if(stStbprpBPList.size() > 0) {
            for (ST_STBPRP_BPojo item : stStbprpBPList) {
                String _stcd=item.getSTCD();
                List<GetWaterViewNewPojo> listTemp = list.stream().filter(p->p.getSTCD().equals(_stcd)).collect(Collectors.toList());
                List<ST_PPTN_RPojo> mListTemp = mList.stream().filter(p->p.getSTCD().equals(_stcd)).collect(Collectors.toList());
                ...
                if(listTemp.size()>0){
                    ...
                    Double maxZ = (new BigDecimal(listTemp.get(0).getUPZ()).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
                ...
                if(mListTemp.size()>0){
                    ...
                    Double maxDrp= (new BigDecimal(mListTemp.get(0).getDRP()).setScale(1, BigDecimal.ROUND_HALF_UP)).doubleValue();
    ...
    public ResultUtils selectListHourlyAreaYB(@RequestBody ColumnName param) {
        ...
        List<Tz_watersheddataPojo> listYB=tzWatersheddataData.selectListLast(etimes,resultStrE,Arrays.asList("48"),"上海气象台");
    ...
    public ResultUtils querySWDRPDANZHANList(@RequestBody ColumnName param){
        ...
        list.forEach(u->{
            ...
            List<ST_PPTN_RPojo> mListT= finalMList.stream().filter(p->p.getSTCD().equals(u.getSTCD())&&p.getTM().equals(u.getTM())).collect(toList());
            if(mListT.size()>0){
...
//        return result;
    ...
    public ResultUtils getEventRain32(@RequestBody ColumnName param) {
        ...
        quList.forEach(item->{
            List<ST_PPTN_RPojo> mListT=mList.stream().filter(o->o.getSTCD().equals(item.getSTCD())).collect(Collectors.toList());
            ...
            // 查询历史场次数据，计算每个站的场次排名
            List<ST_RAINSTORMFREQUENCY_RPojo> historyListT = historyList.stream().filter(o->o.getSTCD().equals(item.getSTCD())).collect(Collectors.toList());
        ...
        if(result.size() > 0){
            // 按雨量倒序排序
            result.sort(Comparator.comparing(ST_PPTN_RPojo::getDRP, Comparator.nullsLast(Double::compareTo)).reversed());
