...
//        List<GetWaterViewNewPojo> waterList = data.selectListByNew(stcdList, stime, etime);
        ...
        for(ST_STBPRP_B_QUPojo quPojo : quList){
            ...
            List<ST_STBPRP_BPojo> collects = stStbprpBList.stream().filter(i ->
                    i.getSTCD().equals(quDto.getSTCD())).collect(Collectors.toList());
            if(collects.size() > 0){
                ...
                //警戒、保证水位
                List<ST_RVFCCH_BPojo> collect1 = rvfcchBList.stream().filter(p -> p.getSTCD().equals(quDto.getSTCD())).collect(Collectors.toList());
                if (collect1.size() > 0) {
...
//            etime = param.getEtime();
//            try {
//                Long time =  new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(etime).getTime() - 24 * 60 * 60 * 1000;
...
//        List<GetWaterViewNewPojo> viewNewList = server.selectListByHisIsDay(stcdList, stime, etime,mtype);
...
//    @ResponseBody
//    public List<Map<String,Object>> dengzhixianWater(@RequestBody DENGZHIXIANYQPojo param){
//        StopWatch watch = new StopWatch();
//        // 计时器开始
//        watch.start();
...
//        String filenameNew = dzxUtils.init(param.getLEVELDATA(),"",FilePathUtils.getRealFilePath(filePath+"/"+filename),param.getLEVELRANGE());
//
...
//    }
    ...
    public List<Map<String,Object>> selectListByMsg(@RequestBody ColumnName param){
        ...
        for(ST_STBPRP_B_QUPojo quPojo : quListWater){
            String _time = etime;
//            List<GetWaterViewNewPojo> mWaterList = list.stream().filter(p -> p.getSTCD().equals(quPojo.getSTCD()) && _time.equals(p.getTM())).collect(Collectors.toList());
            List<GetWaterViewNewPojo> mWaterList = list.stream().filter(p -> p.getSTCD().equals(quPojo.getSTCD())).sorted(Comparator.comparing(GetWaterViewNewPojo::getTM).reversed()).collect(Collectors.toList());
            strMsg.append(quPojo.getSTNM());
            if(null != mWaterList && mWaterList.size() > 0){
                double z = (new BigDecimal(mWaterList.get(0).getUPZ()).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
                DecimalFormat decimalFormat = new DecimalFormat("0.00#");
                strMsg.append((decimalFormat.format(z))+"m");
//                System.out.println(quPojo.getSTNM() +" ： "+ mWaterList.get(0).getTM()+ " = " + (decimalFormat.format(z))+"m");
//                double z = Double.parseDouble(String.format("%.2f",(mWaterList.get(0).getUPZ())));
//                DecimalFormat decimalFormat = new DecimalFormat("0.00#");
//                strMsg.append((decimalFormat.format(z))+"m");
            }else{
                strMsg.append("—m");
...
//
//                List<ST_STBPRP_B_QUPojo> mWaterListQu = quListWater.stream().filter(p -> p.getSTCD().equals(item.getSTCD())).collect(Collectors.toList());
...
////                List<GetWaterViewNewPojo> mWaterListNew = list.stream().filter(p -> p.getSTCD().equals(item.getSTCD()) && p.getTM().equals(_day)).collect(Collectors.toList());
...
//        }
        ...
        try{
            ...
            _stimeRain = DateUtil.dateFormat(new Date(new Date(time).getTime() - 24 * 60 * 60 * 1000), "yyyy-MM-dd 08:05:00");
        ...
        if(stcdList.size() > 0){
            for(String  item : stcdList){
                List<ST_STBPRP_BPojo> collect = stStbprpBPList.stream().filter(i -> i.getSTCD().equals(item)).collect(Collectors.toList());
                if(collect.size() > 0){
                    ...
                    List<ST_PPTN_RPojo> drpList = sprlist.stream().filter(i -> i.getSTCD().equals(item)).collect(Collectors.toList());
                    if(drpList.size() > 0) {
        ...
        if(null != mList && mList.size() > 0) {
            ...
            if(SumDRP > 0){
                List<ST_PPTN_RPojo> mListJBXY = mList.stream().filter(i -> i.getDRP() < 10).collect(Collectors.toList());
                List<ST_PPTN_RPojo> mListJBZY = mList.stream().filter(i -> i.getDRP() < 25 &&  i.getDRP() >= 10).collect(Collectors.toList());
                List<ST_PPTN_RPojo> mListJBDY = mList.stream().filter(i -> i.getDRP() < 50 &&  i.getDRP() >= 25).collect(Collectors.toList());
                List<ST_PPTN_RPojo> mListJBBY = mList.stream().filter(i -> i.getDRP() < 100 &&  i.getDRP() >= 50).collect(Collectors.toList());
                List<ST_PPTN_RPojo> mListJBDBY = mList.stream().filter(i -> i.getDRP() < 250 &&  i.getDRP() >= 100).collect(Collectors.toList());
                List<ST_PPTN_RPojo> mListJBTDBY = mList.stream().filter(i -> i.getDRP() >= 250).collect(Collectors.toList());
//                System.out.println(" mListJBXY.size() = " + mListJBXY.size() + " SumDRP = " + SumDRP);
//                System.out.println(" mListJBZY.size() = " + mListJBZY.size() + " SumDRP = " + SumDRP);
//                System.out.println(" mListJBDY.size() = " + mListJBDY.size() + " SumDRP = " + SumDRP);
//                System.out.println(" mListJBBY.size() = " + mListJBBY.size() + " SumDRP = " + SumDRP);
//                System.out.println(" mListJBDBY.size() = " + mListJBDBY.size() + " SumDRP = " + SumDRP);
//                System.out.println(" mListJBTDBY.size() = " + mListJBTDBY.size() + " SumDRP = " + SumDRP);

                if(null != mListJBTDBY && ((mListJBTDBY.size() < 10 && mListJBTDBY.size() > 0) || mListJBTDBY.size() >=10)){
                ...
                else  if(null != mListJBDBY && ((mListJBDBY.size() < 10 && mListJBDBY.size() > 0) || mListJBDBY.size() > 0)){
                    Double sumDrp = SumDRP / mList.size();
                    Double drp = mList.get(0).getDRP();
                    strRainMsg.append("全市局部暴雨到大暴雨，");
                    strRainMsg.append("面平均雨量"+String.format("%.1f",sumDrp)+"mm，");
                }
                else  if(null != mListJBBY && ((mListJBBY.size() < 10 && mListJBBY.size() > 0) || mListJBBY.size() >= 10)){
                    Double sumDrp = SumDRP / mList.size();
                    Double drp = mList.get(0).getDRP();
                    strRainMsg.append("全市局部大雨到暴雨，");
                    strRainMsg.append("面平均雨量"+String.format("%.1f",sumDrp)+"mm，");
                }
                else  if(null != mListJBDY && ((mListJBDY.size() < 10 && mListJBDY.size() > 0) || mListJBDY.size() >= 10)){
                    Double sumDrp = SumDRP / mList.size();
                    Double drp = mList.get(0).getDRP();
                    strRainMsg.append("全市局部中雨到大雨，");
                    strRainMsg.append("面平均雨量"+String.format("%.1f",sumDrp)+"mm，");
                }
                else  if(null != mListJBZY && ((mListJBZY.size() < 10 && mListJBZY.size() > 0) || mListJBZY.size() >= 10)){
                    Double sumDrp = SumDRP / mList.size();
                    Double drp = mList.get(0).getDRP();
...
//                    strRainMsg.append("面平均雨量"+String.format("%.1f",sumDrp)+"mm，");
        ...
        for(ST_STBPRP_B_QUPojo quPojo : quListLL){
            String _time = etime;
            List<ST_WDPSTAT_RPojo> mWdpstatList = mListLL.stream().filter(p -> p.getSTCD().equals(quPojo.getSTCD()) && _time.equals(p.getIDTM())).collect(Collectors.toList());
            ...
            if(null != mWdpstatList && mWdpstatList.size() > 0){
                ...
                {
                    if((null != mWdpstatList.get(0).getACCPW() && mWdpstatList.get(0).getACCPW() <= 0 )&& (null != mWdpstatList.get(0).getACCDW()   && mWdpstatList.get(0).getACCDW() <= 0)){
                        strWdpstatMsg.append("关闸");
                    }else
                    {
        ...
        Double czUpz = 0.03;
        //昨日开关闸情况
        List<Map<String,Object>> mGateList = dataWaterGateServer.selectListByGate("HOUR","2024030816580613583",stime,etime,czUpz);
        ...
        if(null != mGateList && mGateList.size() > 0){
            ...
            mGateList.forEach((res) ->{
                ...
                //                    List<GetWaterViewNewPojo> waterList = new JSONMapper().readValue(waterListRes,new TypeReference<List<GetWaterViewNewPojo>>(){});
...
//                    System.out.println(" waterList === " + waterList);
                    ...
                    List<GetWaterViewNewPojo> waterListNew = waterList.stream().filter(u -> u.getTM().equals(etimeGate)).collect(Collectors.toList());
...
//        List<GetWaterViewNewPojo> userListRiver=data.selectListByHisIsTime(listSTCD, stime, etime,dayhour,mtype);
//        List<GetWaterViewNewPojo> userListWas=databx.selectListByHisIsTime(listSTCD, stime, etime,dayhour,mtype);
        ...
        if(quList.size()>0){
           for (int num=0;num<quList.size();num++){
               ...
               temponeRiverList=oneRiverList.stream().filter(u->u.getUPZ()!=null).sorted(Comparator.comparing(GetWaterViewNewPojo::getTM).reversed()).collect(Collectors.toList());
    ...
    public List<GetWaterViewNewPojo> queryBySWDanZhan(@RequestBody ColumnName param){
        ...
        if(listSTCD.size()>0){
            for (int num=0;num<listSTCD.size();num++){
                ...
                List<GetWaterViewNewPojo> oneRiverList=userListRiver.stream().filter(u->u.getSTCD().equals(stcd)).collect(Collectors.toList());
                List<GetWaterViewNewPojo> oneWasList=userListWas.stream().filter(u->u.getSTCD().equals(stcd)).collect(Collectors.toList());
                List<GetWaterViewNewPojo> oneTideList=userListTide.stream().filter(u->u.getSTCD().equals(stcd)).collect(Collectors.toList());
                List<GetWaterViewNewPojo> onePumpList=userListPump.stream().filter(u->u.getSTCD().equals(stcd)).collect(Collectors.toList());
                String finalDataSource = mtype;
                if(userListRiver.size()>0){

                    oneRiverList.forEach(u->{
                        GetWaterViewNewPojo pojo=u;
                        List<ST_STBPRP_BPojo> onestStbprpB=stStbprpBList.stream().filter(i->i.getSTCD().equals(u.getSTCD())&&i.getMTYPE().equals(finalDataSource)).collect(Collectors.toList());
                        if(onestStbprpB.size()>0){
                            pojo.setDTPR(onestStbprpB.get(0).getDTPR());
                            if(!CommonUtills.isEmpty(pojo.getDTPR())){
                                String _upz="";
    ...
    public List<GetWaterViewNewPojo> queryBySWQNew(@RequestBody ColumnName param){
        ...
        if(quList.size()>0){
            for (int num=0;num<quList.size();num++){
                ...
                List<GetWaterViewNewPojo> oneRiverList=userListRiver.stream().filter(u->u.getSTCD().equals(stcd)&&!CommonUtills.isEmpty(u.getQ())).collect(Collectors.toList());
                List<GetWaterViewNewPojo> oneWasList=userListWas.stream().filter(u->u.getSTCD().equals(stcd)&&!CommonUtills.isEmpty(u.getQ())).collect(Collectors.toList());
                List<ST_STBPRP_BPojo> onestStbprpB=stStbprpBList.stream().filter(u->u.getSTCD().equals(stcd)).collect(Collectors.toList());
                List<ST_STBPRP_B_QUPojo> onequList=quList.stream().filter(u->u.getSTCD().equals(stcd)).collect(Collectors.toList());
...
//    public List<SingleStationWaterAnalysisPojo> querySingleStationWaterAnalysis(@RequestBody ColumnName param){
//        Date date = new Date(new Date().getTime() - 24 * 60 * 60 * 1000);
//        String dayhour="Minute",stime= DateUtil.dateFormat(date,"yyyy-MM-dd HH:mm:ss"),etime="";
//        List<String> listSTCD=new ArrayList<>();
//        if(null != param.getPathname()){
...
//                if(null != j.getTM()){
//                    try {
//                        return dateFormat.parse(j.getTM()).after(_stm) && dateFormat.parse(j.getTM()).before(_etm);
//                    } catch (ParseException e) {
//                        e.printStackTrace();
//                    }
//                }
...
//                });
//                //平均水位
//                Double avgZ = userListRiverTemp.stream().mapToDouble(item -> Double.valueOf(item.getUPZ())).average().getAsDouble();
//                avgZ= (new BigDecimal(avgZ).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
//                //最高水位
//                Double maxZ = (new BigDecimal(userListRiverTemp.get(0).getUPZ()).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
//                String maxZ_TM=userListRiverTemp.get(0).getTM();
//                //最低水位
//                Double minZ = (new BigDecimal(userListRiverTemp.get(userListRiverTemp.size() - 1).getUPZ()).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
...
//                });
//                //平均水位
//                Double avgZ = userListWasTemp.stream().mapToDouble(item -> Double.valueOf(item.getUPZ())).average().getAsDouble();
//                avgZ= (new BigDecimal(avgZ).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
//                //最高水位
//                Double maxZ = (new BigDecimal(userListWasTemp.get(0).getUPZ()).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
//                String maxZ_TM=userListWasTemp.get(0).getTM();
//                //最低水位
//                Double minZ = (new BigDecimal(userListWasTemp.get(userListWasTemp.size() - 1).getUPZ()).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
...
//                });
//                //平均水位
//                Double avgZ = userListTideTemp.stream().mapToDouble(item -> Double.valueOf(item.getUPZ())).average().getAsDouble();
//                avgZ= (new BigDecimal(avgZ).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
//                //最高水位
//                Double maxZ = (new BigDecimal(userListTideTemp.get(0).getUPZ()).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
//                String maxZ_TM=userListTideTemp.get(0).getTM();
//                //最低水位
//                Double minZ = (new BigDecimal(userListTideTemp.get(userListTideTemp.size() - 1).getUPZ()).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
...
//                });
//                //平均水位
//                Double avgZ = userListPumpTemp.stream().mapToDouble(item -> Double.valueOf(item.getUPZ())).average().getAsDouble();
//                avgZ= (new BigDecimal(avgZ).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
//                //最高水位
//                Double maxZ = (new BigDecimal(userListPumpTemp.get(0).getUPZ()).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
//                String maxZ_TM=userListPumpTemp.get(0).getTM();
//                //最低水位
//                Double minZ = (new BigDecimal(userListPumpTemp.get(userListPumpTemp.size() - 1).getUPZ()).setScale(2, BigDecimal.ROUND_HALF_UP)).doubleValue();
                ...
                waterGroupList.forEach((mWaterList) ->{
                    if(mWaterList.size() > 0){
                        ...
                        for(int numII = 0; numII< finalStcdList.size(); numII++){
                            ...
                            List<GetWaterViewNewPojo> finalListTemp= finalList.stream().filter(p->DateUtil.strToDate(p.getTM(),DateUtil.YMDHMS).getTime()==DateUtil.strToDate(finaTime,DateUtil.YMDHMS).getTime()&&p.getSTCD().equals(_stcd)).collect(Collectors.toList());
...
//        return json;
    ...
    public ResultUtils selectListByHisYB(@RequestBody ColumnName param){
        ...
        //    List<ST_STBPRP_B_STCDPojo>  listB=stbprpBStcdData.selectList(Arrays.asList(param.getStcd().split(",")),Arrays.asList("1"),Arrays.asList("上海水文总站"));
        //    String zstcd=listB.size()>0?listB.get(0).getZSTCD():"";
            listBDMS=bdmsPredictData.selectList(null,null,null,Arrays.asList(solutionid.split(",")),null,id,null,null,null,stcdList);
        }
        ...
        if(listBDMS.size()>0){
            for(GetWaterViewNewPojo item:list){
                ...
                List<BDMS_PREDICTPojo> listBDMSTemp= listBDMS.stream().filter(u->u.getYMDHM().equals(item.getTM())).collect(Collectors.toList());
            ...
            List<BDMS_PREDICTPojo> listBDMSTempS= listBDMS.stream()
                    .filter(u -> {
                        ...
                        return currentTM != null && !currentTM.isEmpty()
                                && (finalLastTM == null || finalLastTM.isEmpty() || currentTM.compareTo(finalLastTM) > 0);
                    })
                    .collect(Collectors.toList());
