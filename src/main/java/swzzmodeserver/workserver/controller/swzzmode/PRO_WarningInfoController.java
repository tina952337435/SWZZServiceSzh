...
import swzzmodeserver.workserver.data.swzzmode.PRO_WarningInfoData;
...
@RestController
@RequestMapping("/SWZZ_MODE_PRO_WarningInfo")
public class PRO_WarningInfoController {
    @Autowired
    private PRO_WarningInfoData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<PRO_WarningInfoPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody PRO_WarningInfoPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,PRO_WarningInfoPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody PRO_WarningInfoPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,PRO_WarningInfoPojo.class))){
            watch.stop();
