...
import swzzmodeserver.workserver.data.swzzmode.COMPUTELISTData;
...
@RestController
@RequestMapping("/SWZZ_MODE_COMPUTELIST")
public class COMPUTELISTController {
    @Autowired
    private COMPUTELISTData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<COMPUTELISTPojo> fxList = data.selectList(ID,key,startindex, Integer.valueOf(pagesize));
        Integer integer = data.selectCount(ID,key);
        Integer count = 1;
        if(!"".equals(pagesize)){
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody COMPUTELISTPojo clPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(clPojo, COMPUTELISTPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        Integer num = data.insertOne(clPojo);
        watch.stop();
        if(num > 0){
    ...
    public ResultUtils modify(@RequestBody COMPUTELISTPojo clPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(clPojo, COMPUTELISTPojo.class))){
            watch.stop();
        ...
        Integer num = data.updateOne(clPojo);
        watch.stop();
        if(num > 0){
            return new ResultUtils<>(num, "操作成功",true, num,watch.getTime());
        }else {
    ...
    public ResultUtils remove(@RequestBody ParamField clPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String ID = "";
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(clPojo, ParamField.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        if(null != clPojo.getStcd()){
            ID = clPojo.getStcd();
        }
...
//    public ResultUtils insertALL(@RequestBody List<COMPUTELISTPojo> clPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
//        Integer num = data.insertALL(clPojo);
//        watch.stop();
//        if(num > 0){
//            return new ResultUtils<>(num, "操作成功",true, num,watch.getTime());
//        }else {
