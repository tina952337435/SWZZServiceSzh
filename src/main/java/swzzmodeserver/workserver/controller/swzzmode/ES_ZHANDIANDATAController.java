...
public class ES_ZHANDIANDATAController {
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo) {
        ...
        List<ES_ZHANDIANDATAPojo> fxList = data.selectList(ID, startindex, Integer.valueOf(pagesize), soid, type,
                startdate, enddate);
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem,
            @RequestBody List<ES_ZHANDIANDATAPojo> bpPojo) {
        StopWatch watch = new StopWatch();
        watch.start();
        String type = "";
        if (CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, ES_ZHANDIANDATAPojo.class))
    ...
    @RequestMapping("/TideLineardifferenceXG")
    public ResultUtils TideLineardifferenceXG(@RequestBody ParamFields bpPojo) {
        StopWatch watch = new StopWatch();
        watch.start();
        String type = "", solutionid = "0", stcd = "", startDate = "", endDate = "";
        ...
        Integer num = service.TideLineardifferenceXG(solutionid, dataList, stcd, type, startDate, endDate);
        watch.stop();
        if (num > 0) {
            return new ResultUtils<>(num, "操作成功", true, num, watch.getTime());
        } else {
    ...
    public ResultUtils MODIFY_MODEZHANDData(@RequestBody ParamFields bpPojo) {
        ...
        Integer num = service.MODIFY_MODEZHANDData(startdate, enddate, solutionid, jydatatype, gcdatatype, scwdatatype,
                username);
        watch.stop();
        if (num > 0) {
            return new ResultUtils<>(num, "操作成功", true, num, watch.getTime());
    ...
    @RequestMapping("/modify_byTMJY")
    public ResultUtils modify_byTMJY(@RequestBody ParamFields bpPojo) {
        StopWatch watch = new StopWatch();
        watch.start();
        String dd_id = "", DayHour = "", ZhanData = "", TM = "";
        List<String> ZhanID = new ArrayList<>();
        ...
        Integer integer = service.modify_byTMJY(TM, dd_id, ZhanID, DayHour, ZhanData);
        watch.stop();
        if (integer > 0) {
            return new ResultUtils<>(null, "操作成功", true, integer, watch.getTime());
        } else {
    ...
    public ResultUtils SWZZ_FLOODTIDEDATA_ST_TIDEHIGH_RSel(@RequestBody ParamFields bpPojo) {
        ...
        List<ST_TIDEHIGHParam> tidehighParamList = service.SWZZ_FLOODTIDEDATA_ST_TIDEHIGH_RSel(stcd, stime, etime,
                tdptn);
        watch.stop();
    ...
    @RequestMapping("/modelSetTaskSZH")
    public ResultUtils modelSetTaskSZH(@RequestBody ParamFields bpPojo) {
        StopWatch watch = new StopWatch();
        watch.start();
        String stime = "", etime = "", dd_id = "";
        int num = 0;
        List<String> ddobj = new ArrayList<>();
        if (CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ParamFields.class))) {
            watch.stop();
        ...
        String result = huishuiApiService.modelSetTaskNew(stime, etime, num, dd_id, null);
        watch.stop();
        return new ResultUtils<>(null, result, true, 0, watch.getTime());
    }
