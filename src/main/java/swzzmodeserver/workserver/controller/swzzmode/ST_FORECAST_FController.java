...
public class ST_FORECAST_FController {
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<ST_FORECAST_FPojo> bpPojo) {
        ...
        watch.start();
        String type = "";
        if (CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, ST_FORECAST_FPojo.class))
    ...
    public ResultUtils SwicSQYBCG_XY(@RequestBody ParamField bpPojo) {
        ...
        for (Map<String, Object> dataItem : dataList) {
            // {"XX":-32188.5441,"YY":-49501.7422,"YY2000":30.7885,"YIJUTIME":"2025-07-22
            // 08:00:00","DATETIME":"2025-07-22
            // 08:00:00","STATIONID":"TH89999942","FABUTIME":"2025-07-22
            // 08:36:12","XX2000":121.1307,"STATIONNAME":"马家宅","DATASOURCE":"太湖局","VALUE":"3.11","TYPE":"水情预报成果"}
            Object stationIdObj = dataItem.get("STATIONID");
            Object yijuTimeObj = dataItem.get("YIJUTIME");
            Object dateTimeObj = dataItem.get("DATETIME");
            Object fabuTimeObj = dataItem.get("FABUTIME");
            Object dataSourceObj = dataItem.get("DATASOURCE");
            Object valueObj = dataItem.get("VALUE");
            if (stationIdObj == null || yijuTimeObj == null || dateTimeObj == null
                    || fabuTimeObj == null || dataSourceObj == null || valueObj == null) {
                continue;
            }
            String STCD = stationIdObj.toString();// 站码
            String FYMDH = yijuTimeObj.toString();// 发布时间
            String YMDH = dateTimeObj.toString();// 发生时间
            String IYMDH = fabuTimeObj.toString();// 预报时间
            String UNITNAME = dataSourceObj.toString();// 发布单位
            String PLCD = stationIdObj.toString();// 原来的站码
            double Z = Double.parseDouble(valueObj.toString()) - 0.26;// 基面转换
        ...
        if (list.size() > 0) {
            // 入库前先删旧数据
            String fymdh = list.get(0).getFYMDH();
            data.deleteByFymdh(fymdh);
    ...
    public ResultUtils SwicSQYBCG(@RequestBody ParamField bpPojo) {
        ...
        try {
            ...
            if (dataList.size() > 0) {
                ...
                if (listFP.size() > 0) {
                    System.out.println(FYMDH + "已存在预报数据，先删后插更新");
                }
                String STARTTIME = FYMDH;
                    ...
                    listGuan.forEach(es_modelguanlianPojo -> {
                        ...
                        // dataListGC的格式：{"XX":-32188.5441,"YY":-49501.7422,"YY2000":30.7885,"YIJUTIME":"2025-07-22
                        // 08:00:00","DATETIME":"2025-07-22
                        // 08:00:00","STATIONID":"TH89999942","FABUTIME":"2025-07-22
                        // 08:36:12","XX2000":121.1307,"STATIONNAME":"马家宅","DATASOURCE":"太湖局","VALUE":"3.11","TYPE":"水情预报成果"}
                        List<Map<String, Object>> dataListGC = swicApiService.getSwicSQYBCG(
                                es_modelguanlianPojo.getSTCD(), es_modelguanlianPojo.getFIELD(), STARTTIME, ENDTIME,
                                FYMDH);
                        if (dataListGC == null || dataListGC.isEmpty()) {
                            return;
                        }
                        // 按 DT_FABUTIME 分组，只取最新的那组
                        Map<String, List<Map<String, Object>>> groupedByFabuTime = new HashMap<>();
                        for (Map<String, Object> item : dataListGC) {
                            Object fabuTimeObj = item.get("DT_FABUTIME");
                            if (fabuTimeObj == null) {
                                continue;
                            }
                            String fabuTime = fabuTimeObj.toString();
                            groupedByFabuTime.computeIfAbsent(fabuTime, k -> new ArrayList<>()).add(item);
                        }
                        if (groupedByFabuTime.isEmpty()) {
                            return;
                        }
                        // 找到 DT_FABUTIME 最大的分组
                        String latestFabuTime = null;
                        for (String fabuTime : groupedByFabuTime.keySet()) {
                            if (latestFabuTime == null || fabuTime.compareTo(latestFabuTime) > 0) {
                                latestFabuTime = fabuTime;
                            }
                        }
                        List<Map<String, Object>> latestGroup = groupedByFabuTime.get(latestFabuTime);
                        // 1小时一个数据，需要插值为5分钟的
                        List<Map<String, Object>> dataListGC5 = LinearInterpolationUtil.interpolateTideData(latestGroup,
                                "DT_TIME", "CHAOWEI_VALUE");
                        // 遍历 data 列表
                        for (Map<String, Object> dataItem : dataListGC5) {
                            Object dtTimeObj = dataItem.get("DT_TIME");
                            Object chaoweiValueObj = dataItem.get("CHAOWEI_VALUE");
                            if (dtTimeObj == null || chaoweiValueObj == null) {
                                continue;
                            }
                            String YMDH = dtTimeObj.toString();// 发生时间
                            double Z = Double.parseDouble(chaoweiValueObj.toString()) - 0.26;// 基面转换
                            if (Double.isNaN(Z) || Double.isInfinite(Z)) {
                                continue;
                            }
                            ST_FORECAST_FPojo pojo = new ST_FORECAST_FPojo();
                    ...
                    // 入库前先删旧数据，确保新的 DT_FABUTIME 数据替换旧的
                    if (list.size() > 0) {
                        data.deleteByFymdh(FYMDH);
                    }
            }
