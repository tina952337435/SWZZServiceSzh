...
import swzzmodeserver.workserver.data.swzzmode.HD_FLOWLINEData;
...
@RestController
@RequestMapping("/SWZZ_MODE_HD_FLOWLINE")
public class HD_FLOWLINEController {
    @Autowired
    private HD_FLOWLINEData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<HD_FLOWLINEPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody HD_FLOWLINEPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,HD_FLOWLINEPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody HD_FLOWLINEPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,HD_FLOWLINEPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<HD_FLOWLINEPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
