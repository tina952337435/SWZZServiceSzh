...
@RestController
@RequestMapping("/SWZZ_MODE_ES_SLTONGJI")
public class ES_SLTONGJIController {
    @Autowired
    private ES_SLTONGJIData data;

    @RequestMapping("/findResult")
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String ID = "",key = "",pid = "",pageindex = "",pagesize = "10";
        String type = "";
        ...
        List<ES_SLTONGJIPojo> fxList = data.selectList(ID,type,pid,startindex, Integer.valueOf(pagesize));
        Integer integer = data.selectCount(ID,type,pid);
        Integer count = 1;
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_SLTONGJIPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_SLTONGJIPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_SLTONGJIPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_SLTONGJIPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_SLTONGJIPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
