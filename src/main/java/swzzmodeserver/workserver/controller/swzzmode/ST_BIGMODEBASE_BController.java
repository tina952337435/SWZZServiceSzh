...
import swzzmodeserver.workserver.data.swzzmode.ST_BIGMODEBASE_BData;
...
@RestController
@RequestMapping("/SWZZ_MODE_st_bigmodebase_b")
public class ST_BIGMODEBASE_BController {
    @Autowired
    private ST_BIGMODEBASE_BData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ST_BIGMODEBASE_BPojo> fxList = data.selectList(ID,key,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ST_BIGMODEBASE_BPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ST_BIGMODEBASE_BPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ST_BIGMODEBASE_BPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ST_BIGMODEBASE_BPojo.class))){
            watch.stop();
