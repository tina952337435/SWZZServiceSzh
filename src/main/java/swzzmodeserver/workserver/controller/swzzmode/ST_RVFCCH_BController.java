...
@RestController
@RequestMapping("/SWZZ_MODE_st_rvfcch_b")
public class ST_RVFCCH_BController {
    @Autowired
    private ST_RVFCCH_BData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ST_RVFCCH_BPojo> fxList = data.selectList(ID,stime,etime,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ST_RVFCCH_BPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ST_RVFCCH_BPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ST_RVFCCH_BPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ST_RVFCCH_BPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ST_RVFCCH_BPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
