...
import swzzmodeserver.workserver.data.swzzmode.SDE_AREA6HOURData;
import swzzmodeserver.workserver.data.swzzmode.SDE_AREAData;
...
@RestController
@RequestMapping("/SWZZ_MODE_SDE_AREA6HOUR")
public class SDE_AREA6HOURController {
    @Autowired
    private SDE_AREA6HOURData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        if(null != bpPojo.getStcd()){
            ID = Integer.valueOf(bpPojo.getStcd());
        }
        ...
        List<SDE_AREA6HOURPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody SDE_AREA6HOURPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,SDE_AREA6HOURPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody SDE_AREA6HOURPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,SDE_AREA6HOURPojo.class))){
            watch.stop();
    ...
    public ResultUtils remove(@RequestBody ParamField sdeAreaPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        Integer ID = null;
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(sdeAreaPojo,ParamField.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        if(null != sdeAreaPojo.getStcd()){
            ID = Integer.valueOf(sdeAreaPojo.getStcd());
        }
...
//    public ResultUtils insertALL(@RequestBody List<SDE_AREA6HOURPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
