...
@RestController
@RequestMapping("/SWZZ_MODE_SDE_AREA")
public class SDE_AREAController {
    @Autowired
    private SDE_AREAData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<SDE_AREAPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody SDE_AREAPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,SDE_AREAPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody SDE_AREAPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,SDE_AREAPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<SDE_AREAPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
