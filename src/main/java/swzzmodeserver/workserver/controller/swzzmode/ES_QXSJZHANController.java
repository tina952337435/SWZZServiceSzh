...
import swzzmodeserver.workserver.data.swzzmode.ES_QXSJZHANData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_QXSJZHAN")
public class ES_QXSJZHANController {
    @Autowired
    private ES_QXSJZHANData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_QXSJZHANPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_QXSJZHANPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_QXSJZHANPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_QXSJZHANPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_QXSJZHANPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_QXSJZHANPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
