...
@RestController
@RequestMapping("/SWZZ_MODE_ES_MODELGUANLIAN")
public class ES_MODELGUANLIANController {
    @Autowired
    private ES_MODELGUANLIANData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_MODELGUANLIANPojo> fxList = data.selectList(ID,key,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_MODELGUANLIANPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_MODELGUANLIANPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_MODELGUANLIANPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_MODELGUANLIANPojo.class))){
            watch.stop();
    ...
    public ResultUtils remove(@RequestBody ParamField bpPojo){
        ...
        if(null != bpPojo.getPattem()){
            key = bpPojo.getPattem();
        }
        Integer num = data.deleteOne(ID,key);
        watch.stop();
        if(num > 0){
            return new ResultUtils<>(num, "操作成功",true, num,watch.getTime());
        }else {
...
//    public ResultUtils insertALL(@RequestBody List<ES_MODELGUANLIANPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
