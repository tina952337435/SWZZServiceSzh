...
import swzzmodeserver.workserver.data.swzzmode.ES_ZHANDIANPXData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_ZHANDIANPX")
public class ES_ZHANDIANPXController {
    @Autowired
    private ES_ZHANDIANPXData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_ZHANDIANPXPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_ZHANDIANPXPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ES_ZHANDIANPXPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_ZHANDIANPXPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ES_ZHANDIANPXPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_ZHANDIANPXPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
