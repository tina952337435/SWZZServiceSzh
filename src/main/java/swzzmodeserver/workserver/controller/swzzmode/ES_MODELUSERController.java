...
import swzzmodeserver.workserver.data.swzzmode.ES_MODELUSERData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_MODELUSER")
public class ES_MODELUSERController {
    @Autowired
    private ES_MODELUSERData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_MODELUSERPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_MODELUSERPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_MODELUSERPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_MODELUSERPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_MODELUSERPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_MODELUSERPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
