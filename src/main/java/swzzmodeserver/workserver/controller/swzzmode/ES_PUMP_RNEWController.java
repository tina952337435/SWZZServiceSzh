...
import swzzmodeserver.workserver.data.swzzmode.ES_PUMP_RNEWData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_PUMP_RNEW")
public class ES_PUMP_RNEWController {
    @Autowired
    private ES_PUMP_RNEWData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_PUMP_RNEWPojo> fxList = data.selectList(ID,stime,etime,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_PUMP_RNEWPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_PUMP_RNEWPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_PUMP_RNEWPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_PUMP_RNEWPojo.class))){
            watch.stop();
