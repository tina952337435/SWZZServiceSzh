...
public class BDMS_PREDICTController {
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo) {
        ...
        if (null != bpPojo.getPid()) {
            ...
            String stimeF = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
                    .format(new Date().getTime() - 72 * 60 * 60 * 1000);
            // 查询最新的
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<BDMS_PREDICTPojo> bpPojo) {
        ...
        watch.start();
        String type = "";
        if (CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, BDMS_PREDICTPojo.class))
    ...
    public ResultUtils WJ_MODELSINGRESULT(@RequestBody ParamFields bpPojo) {
        ...
        if (null != bpPojo.getSolutionid()) {
            ...
            List<DD_SOLUTIONPojo> ListDDTop = ddSolutionData.selectListNew(null, bpPojo.getStartdate(),
                    bpPojo.getEnddate());
            if (ListDDTop.size() > 0) {
    ...
    public ResultUtils MODE_BDMS_PREDICTSelMORE(@RequestBody ParamFields bpPojo) {
        ...
        List<Map<String, String>> mapList = service.MODE_BDMS_PREDICTSelMORE(stime, etime,
                Arrays.asList(plan_n.split(",")), stcd, data_type);
        watch.stop();
        if (mapList.size() > 0) {
