...
import swzzmodeserver.workserver.data.swzzmode.St_AstronomicalTide_BData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ST_ASTRONOMICALTIDE_R")
public class ST_ASTRONOMICALTIDE_RController {
    @Autowired
    private ST_ASTRONOMICALTIDE_RData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ST_ASTRONOMICALTIDE_RPojo> fxList = data.selectList(listID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
        Integer integer = data.selectCount(Collections.singletonList(ID),key,stime,etime,type);
        Integer count = 1;
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ST_ASTRONOMICALTIDE_RPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ST_ASTRONOMICALTIDE_RPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ST_ASTRONOMICALTIDE_RPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ST_ASTRONOMICALTIDE_RPojo.class))){
            watch.stop();
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<ST_ASTRONOMICALTIDE_RPojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String type = "";
        if(CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo,ST_ASTRONOMICALTIDE_RPojo.class)) || !FieldIsValid.isValid(pattem)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        if(null != pattem){
        ...
        List<ST_ASTRONOMICALTIDE_RPojo> list = new ArrayList<>();
