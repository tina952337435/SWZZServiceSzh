...
import swzzmodeserver.workserver.data.swzzmode.ES_MODELFANGANData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_MODELFANGAN")
public class ES_MODELFANGANController {
    @Autowired
    private ES_MODELFANGANData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_MODELFANGANPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_MODELFANGANPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_MODELFANGANPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_MODELFANGANPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_MODELFANGANPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_MODELFANGANPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
