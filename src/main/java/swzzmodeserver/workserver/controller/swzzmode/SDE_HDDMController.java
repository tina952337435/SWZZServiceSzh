...
import swzzmodeserver.workserver.data.swzzmode.SDE_HDDMData;
...
@RestController
@RequestMapping("/SWZZ_MODE_SDE_HDDM")
public class SDE_HDDMController {
    @Autowired
    private SDE_HDDMData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<SDE_HDDMPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody SDE_HDDMPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,SDE_HDDMPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody SDE_HDDMPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,SDE_HDDMPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<SDE_HDDMPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
