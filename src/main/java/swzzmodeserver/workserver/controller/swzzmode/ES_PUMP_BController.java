...
import swzzmodeserver.workserver.data.swzzmode.ES_PUMP_BData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_PUMP_B")
public class ES_PUMP_BController {
    @Autowired
    private ES_PUMP_BData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_PUMP_BPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_PUMP_BPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_PUMP_BPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_PUMP_BPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_PUMP_BPojo.class))){
            watch.stop();
