...
import swzzmodeserver.workserver.data.swzzmode.ES_MODELFANGANZHANData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_MODELFANGANZHAN")
public class ES_MODELFANGANZHANController {
    @Autowired
    private ES_MODELFANGANZHANData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_MODELFANGANZHANPojo> fxList = data.selectList(ID,null,startindex, Integer.valueOf(pagesize));
        Integer integer = data.selectCount(ID,null);
        Integer count = 1;
        if(!"".equals(pagesize)){
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_MODELFANGANZHANPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_MODELFANGANZHANPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_MODELFANGANZHANPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_MODELFANGANZHANPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_MODELFANGANZHANPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
