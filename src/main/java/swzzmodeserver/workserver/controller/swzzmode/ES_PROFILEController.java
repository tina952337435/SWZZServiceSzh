...
import swzzmodeserver.workserver.data.swzzmode.ES_PROFILEData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_PROFILE")
public class ES_PROFILEController {
    @Autowired
    private ES_PROFILEData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_PROFILEPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_PROFILEPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_PROFILEPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_PROFILEPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_PROFILEPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_PROFILEPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
