...
import swzzmodeserver.workserver.data.swzzmode.ES_FEATURESData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_FEATURES")
public class ES_FEATURESController {
    @Autowired
    private ES_FEATURESData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_FEATURESPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_FEATURESPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ES_FEATURESPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_FEATURESPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ES_FEATURESPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_FEATURESPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
