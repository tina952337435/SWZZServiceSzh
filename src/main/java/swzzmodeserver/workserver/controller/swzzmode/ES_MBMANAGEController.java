...
import swzzmodeserver.workserver.data.swzzmode.ES_MBMANAGEData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_MBMANAGE")
public class ES_MBMANAGEController {
    @Autowired
    private ES_MBMANAGEData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_MBMANAGEPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_MBMANAGEPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_MBMANAGEPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_MBMANAGEPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_MBMANAGEPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_MBMANAGEPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
