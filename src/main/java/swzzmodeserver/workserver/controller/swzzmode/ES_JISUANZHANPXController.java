...
@RestController
@RequestMapping("/SWZZ_MODE_ES_JISUANZHANPX")
public class ES_JISUANZHANPXController {
    @Autowired
    private ES_JISUANZHANPXData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_JISUANZHANPXPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_JISUANZHANPXPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_JISUANZHANPXPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_JISUANZHANPXPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_JISUANZHANPXPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_JISUANZHANPXPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
