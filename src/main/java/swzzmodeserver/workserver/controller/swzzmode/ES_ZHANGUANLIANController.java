...
@RestController
@RequestMapping("/SWZZ_MODE_ES_ZHANGUANLIAN")
public class ES_ZHANGUANLIANController {
    @Autowired
    private ES_ZHANGUANLIANData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_ZHANGUANLIANPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize),type);
        Integer integer = data.selectCount(ID,type);
        Integer count = 1;
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_ZHANGUANLIANPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_ZHANGUANLIANPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_ZHANGUANLIANPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_ZHANGUANLIANPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_ZHANGUANLIANPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
