...
import swzzmodeserver.workserver.data.swzzmode.ES_RAINFALLData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_RAINFALL")
public class ES_RAINFALLController {
    @Autowired
    private ES_RAINFALLData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        if(null != bpPojo.getStcd()){
            String[] strings = bpPojo.getStcd().split(",");
            if (strings.length > 1){
                zhanidList = Arrays.asList();
        ...
        List<ES_RAINFALLPojo> fxList = data.selectList(ID,type,key,zhanidList,startindex, Integer.valueOf(pagesize));
        Integer integer = data.selectCount(ID,type,key,zhanidList);
        Integer count = 1;
        if(!"".equals(pagesize)){
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_RAINFALLPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_RAINFALLPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_RAINFALLPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_RAINFALLPojo.class))){
            watch.stop();
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<ES_RAINFALLPojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String type = "";
        if(CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo,ES_RAINFALLPojo.class)) || !FieldIsValid.isValid(pattem)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        if(null != pattem){
        ...
        List<ES_RAINFALLPojo> list = new ArrayList<>();
