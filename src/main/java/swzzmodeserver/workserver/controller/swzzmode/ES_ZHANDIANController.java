...
public class ES_ZHANDIANController {
    ...
    @Autowired
    private ST_RNFL_RLISTData RLISTData;
    @Autowired
    private ST_RNFL_RMODEData RLMODEData;
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<ES_ZHANDIANPojo> bpPojo) {
        ...
        watch.start();
        String type = "";
        if (CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, ES_ZHANDIANPojo.class))
    ...
    public ResultUtils MODIFYJYYBBY(@RequestBody ParamFields bpPojo) {
        ...
        // List<Tz_watersheddataPojo> tzWatersheddataPojoList =
        // tzWatersheddataData.selectByTimeAndFPDR(startdate, enddate,
        // format.format(new Date(rldate.getTime() - 3 * 24 * 60 * 60 * 1000)),
        // format.format(rldate),
        // fpdrList);
        List<Tz_watersheddataPojo> tzWatersheddataPojoList = tzWatersheddataData.selectListLastByID(startdate, enddate,
                fpdrList, "上海气象台", format.format(new Date(rldate.getTime() - 3 * 24 * 60 * 60 * 1000)),
                format.format(rldate));
        List<ES_ZHANDIANPojo> esZhandianPojoList = data.selectList("", null, null, Collections.singletonList("0"), "");
        List<String> zhanidList = esZhandianPojoList.stream().map(ES_ZHANDIANPojo::getZHANID)
                .collect(Collectors.toList());
        List<ES_ZHANDIANDATAPojo> esZhandiandataPojoList = esZhandiandataData.selectList("", null, null, solutionid,
                null, null, null);
        ...
        for (String id : zhanidList) {
            List<ES_ZHANDIANDATAPojo> ZHANDIANDATAFilter = esZhandiandataPojoList.stream()
            ...
            ZHANDIANDATAFilter.forEach(n -> {
                List<Tz_watersheddataPojo> tzFilter = tzWatersheddataPojoList.stream().filter(t -> {
                    if (t.getKEYID() != null && n.getZHANID() != null && t.getFTM() != null
                            && n.getZHANTIME() != null) {
    ...
    @RequestMapping("/MODIFYJYYBBY_Beixuan")
    public ResultUtils MODIFYJYYBBY_Beixuan(@RequestBody ParamFields bpPojo) {
        StopWatch watch = new StopWatch();
        watch.start();
        if (CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ParamFields.class))) {
            watch.stop();
        ...
        List<ES_SLTONGJIPojo> esSltongjiList = esSltongjIData.selectList(null, "134", null, null, null);
        List<ST_RNFL_RMODEPojo> tzWatersheddataPojoList = RLMODEData.selectListLastBySTCD(null, startdate, enddate,
                fpdrList, "水利分片");
        // List<ES_ZHANDIANPojo> esZhandianPojoList = data.selectList("", null, null,
        // Collections.singletonList("0"), "");
        // List<String> zhanidList =
        // esZhandianPojoList.stream().map(ES_ZHANDIANPojo::getZHANID)
        // .collect(Collectors.toList());
        // List<ES_ZHANDIANDATAPojo> esZhandiandataPojoList =
        // esZhandiandataData.selectList("", null, null, solutionid,
        // null, null, null);
        // esZhandiandataPojoList = esZhandiandataPojoList.stream().filter(n -> {
        // if (n.getZHANID() != null) {
        // return zhanidList.contains(n.getZHANID());
        ...
        // List<ES_ZHANDIANDATAPojo> esZhandianDataList = new ArrayList<>();
        // for (String id : zhanidList) {
        // List<ES_ZHANDIANDATAPojo> ZHANDIANDATAFilter =
        // esZhandiandataPojoList.stream()
        // .filter(m -> m.getZHANID().equals(id)).collect(Collectors.toList());

        // List<ES_SLTONGJIPojo> esSltongjiListT = esSltongjiList.stream().filter(u ->
        // u.getSTCD().contains(id))
        // .collect(Collectors.toList());

        // List<ST_RNFL_RMODEPojo> tzWatersheddataPojoListTemp = new ArrayList<>();
        // if (esSltongjiListT.size() > 0) {
        // String slpName = esSltongjiListT.get(0).getFX();
        // tzWatersheddataPojoListTemp = tzWatersheddataPojoList.stream().filter(u ->
        // u.getSTCD().equals(slpName))
        // .collect(Collectors.toList());
        // }
        // for (int i = 0; i < ZHANDIANDATAFilter.size(); i++) {
        // ES_ZHANDIANDATAPojo n = ZHANDIANDATAFilter.get(i);
        // List<ST_RNFL_RMODEPojo> tzFilter = tzWatersheddataPojoListTemp.stream()
        // .filter(t -> t.getTM().equals(n.getZHANTIME())).collect(Collectors.toList());
        // Double DRP = 0.0;
        // if (tzFilter.size() > 0) {
        // DRP = tzFilter.get(0).getDRP();
        // }
