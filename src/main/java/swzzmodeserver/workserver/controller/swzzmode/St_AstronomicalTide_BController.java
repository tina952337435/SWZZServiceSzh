...
@RestController
@RequestMapping("/SWZZ_MODE_st_AstronomicalTide_B")
public class St_AstronomicalTide_BController {
    @Autowired
    private St_AstronomicalTide_BData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<St_AstronomicalTide_BPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody St_AstronomicalTide_BPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,St_AstronomicalTide_BPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody St_AstronomicalTide_BPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,St_AstronomicalTide_BPojo.class))){
            watch.stop();
...
//    @RequestMapping("/batchResult")
//    public ResultUtils batchResult(String pattem,@RequestBody List<St_AstronomicalTide_BPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
//        String type = "";
//        if(null != pattem){
...
//        Integer num = 0;
//        if(type.equals("true") ){
//            for(St_AstronomicalTide_BPojo QxObj: bpPojo){
//                num += data.updateOne(QxObj);
//            }
//        }else {
//            num = data.insertALL(bpPojo);
//        }
//        watch.stop();
//        if(num > 0){
//            return new ResultUtils<>(num, "操作成功",true, num,watch.getTime());
...
//    public ResultUtils batchRemove(String pattem,@RequestBody List<St_AstronomicalTide_BPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
//        Integer num = data.deleteALL(bpPojo);
//        watch.stop();
//        if(num > 0){
//            return new ResultUtils<>(num, "操作成功",true, num,watch.getTime());
//        }else {
