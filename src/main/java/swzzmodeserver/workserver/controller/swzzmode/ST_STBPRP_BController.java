...
@RestController
@RequestMapping("/SWZZ_MODE_st_stbprp_b")
public class ST_STBPRP_BController {
    @Autowired
    private ST_STBPRP_BData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ST_STBPRP_BPojo> fxList = data.selectList(ID,key,stime,etime,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ST_STBPRP_BPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ST_STBPRP_BPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ST_STBPRP_BPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ST_STBPRP_BPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ST_STBPRP_BPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
