...
import swzzmodeserver.workserver.data.swzzmode.ES_RAINFALL_TREEData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ES_RAINFALL_TREE")
public class ES_RAINFALL_TREEController {
    @Autowired
    private ES_RAINFALL_TREEData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ES_RAINFALL_TREEPojo> fxList = data.selectList(ID,key,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ES_RAINFALL_TREEPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_RAINFALL_TREEPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ES_RAINFALL_TREEPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ES_RAINFALL_TREEPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ES_RAINFALL_TREEPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
