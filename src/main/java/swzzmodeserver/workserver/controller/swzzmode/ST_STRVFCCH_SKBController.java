...
import swzzmodeserver.workserver.data.swzzmode.ST_STRVFCCH_SKBData;
...
@RestController
@RequestMapping("/SWZZ_MODE_ST_STRVFCCH_SKB")
public class ST_STRVFCCH_SKBController {
    @Autowired
    private ST_STRVFCCH_SKBData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ST_STRVFCCH_SKBPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ST_STRVFCCH_SKBPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ST_STRVFCCH_SKBPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ST_STRVFCCH_SKBPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,ST_STRVFCCH_SKBPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<ST_STRVFCCH_SKBPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
