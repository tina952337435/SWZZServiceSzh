...
import swzzmodeserver.workserver.data.swzzmode.DD_SOLUTION_YSData;
...
@RestController
@RequestMapping("/SWZZ_MODE_DD_SOLUTION_YS")
public class DD_SOLUTION_YSController {
    @Autowired
    private DD_SOLUTION_YSData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<DD_SOLUTION_YSPojo> fxList = data.selectList(ID,startindex, Integer.valueOf(pagesize));
        Integer integer =fxList.size();// data.selectCount(ID);
        Integer count = 1;
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody DD_SOLUTION_YSPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, DD_SOLUTION_YSPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody DD_SOLUTION_YSPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, DD_SOLUTION_YSPojo.class))){
            watch.stop();
...
//    public ResultUtils insertALL(@RequestBody List<DD_SOLUTION_YSPojo> bpPojo){
//        StopWatch watch = new StopWatch();
//        watch.start();
