...
@RestController
@RequestMapping("/SWZZ_DATA_myicon")
public class MyiconController {
    @Autowired
    private MyiconData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<MyiconPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody MyiconPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        MyiconPojo params = FieldIsValid.getColumnName(bpPojo,MyiconPojo.class);
        if(CommonUtills.isEmpty(params)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody MyiconPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        MyiconPojo params = FieldIsValid.getColumnName(bpPojo,MyiconPojo.class);
        if(CommonUtills.isEmpty(params)){
