...
@RestController
@RequestMapping("/SWZZ_DATA_sys_log")
public class Sys_logController {
    @Autowired
    private Sys_logData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<Sys_logPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody Sys_logPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,Sys_logPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody Sys_logPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,Sys_logPojo.class))){
            watch.stop();
