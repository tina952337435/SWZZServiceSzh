...
import swzzmodeserver.workserver.data.swzzdata.MyiconData;
import swzzmodeserver.workserver.data.swzzdata.Qx_groupData;
...
@RestController
@RequestMapping("/SWZZ_DATA_qx_group")
public class Qx_groupController {
    @Autowired
    private Qx_groupData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<Qx_groupPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody Qx_groupPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        Qx_groupPojo params = FieldIsValid.getColumnName(bpPojo,Qx_groupPojo.class);
        if(CommonUtills.isEmpty(params)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody Qx_groupPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        Qx_groupPojo params = FieldIsValid.getColumnName(bpPojo,Qx_groupPojo.class);
        if(CommonUtills.isEmpty(params)){
