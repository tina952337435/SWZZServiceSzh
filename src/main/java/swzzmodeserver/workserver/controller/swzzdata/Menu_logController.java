...
@RestController
@RequestMapping("/SWZZ_DATA_menu_log")
public class Menu_logController {
    @Autowired
    private Menu_logData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<Menu_logPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody Menu_logPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        Menu_logPojo params = FieldIsValid.getColumnName(bpPojo,Menu_logPojo.class);
        if(CommonUtills.isEmpty(params)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody Menu_logPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        Menu_logPojo params = FieldIsValid.getColumnName(bpPojo,Menu_logPojo.class);
        if(CommonUtills.isEmpty(params)){
