...
import swzzmodeserver.workserver.data.swzzdata.SmsData;
...
@RestController
@RequestMapping("/SWZZ_DATA_st_formula_b")
public class St_formula_bController {
    @Autowired
    private St_formula_bData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<St_formula_bPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody St_formula_bPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,St_formula_bPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody St_formula_bPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,St_formula_bPojo.class))){
            watch.stop();
