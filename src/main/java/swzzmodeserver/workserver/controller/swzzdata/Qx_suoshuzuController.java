...
import swzzmodeserver.workserver.data.swzzdata.Qx_suoshuzuData;
...
@RestController
@RequestMapping("/SWZZ_DATA_qx_suoshuzu")
public class Qx_suoshuzuController {
    @Autowired
    private Qx_suoshuzuData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<Qx_suoshuzuPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody Qx_suoshuzuPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,Qx_suoshuzuPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody Qx_suoshuzuPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,Qx_suoshuzuPojo.class))){
            watch.stop();
