...
import swzzmodeserver.workserver.data.swzzdata.St_formula_bData;
import swzzmodeserver.workserver.data.swzzdata.St_formula_rData;
...
@RestController
@RequestMapping("/SWZZ_DATA_st_formula_r")
public class St_formula_rController {
    @Autowired
    private St_formula_rData data;

    @RequestMapping("/findResult")
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String ID = "",key = "",pageindex = "",pagesize = "10",pid = "";
        ...
        if(null != bpPojo.getStrExp()){
            pid = bpPojo.getStrExp();
        }
        Integer startindex = null;
        if(!"".equals(pageindex) && !"".equals(pagesize)){
            startindex = (Integer.parseInt(pageindex) - 1) * Integer.parseInt(pagesize);
        }
        List<St_formula_rPojo> fxList = data.selectList(ID,key,stime,etime,type,pid,startindex, Integer.valueOf(pagesize));
        Integer integer = data.selectCount(ID,key,stime,etime,type,pid);
        Integer count = 1;
        if(!"".equals(pagesize)){
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody St_formula_rPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,St_formula_rPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody St_formula_rPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,St_formula_rPojo.class))){
            watch.stop();
