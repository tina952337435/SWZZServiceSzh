...
@RestController
@RequestMapping("/SWZZ_DATA_qx_enable")
public class Qx_enableController {
    @Autowired
    private Qx_enableData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<Qx_enablePojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody Qx_enablePojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        Qx_enablePojo params = FieldIsValid.getColumnName(bpPojo,Qx_enablePojo.class);
        if(CommonUtills.isEmpty(params)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody Qx_enablePojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        Qx_enablePojo params = FieldIsValid.getColumnName(bpPojo,Qx_enablePojo.class);
        if(CommonUtills.isEmpty(params)){
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<Qx_enablePojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String type = "";
        List<Qx_enablePojo> params = FieldIsValid.getListColumnName(bpPojo,Qx_enablePojo.class);
        if(CommonUtills.isEmpty(params) || !FieldIsValid.isValid(pattem)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        if(null != pattem){
        ...
        List<Qx_enablePojo> list = new ArrayList<>();
