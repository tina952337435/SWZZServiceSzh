...
import swzzmodeserver.workserver.data.swzzdata.Sys_reportlogData;
import swzzmodeserver.workserver.pojo.swzzdata.Sys_logPojo;
import swzzmodeserver.workserver.pojo.swzzdata.Sys_reportlogPojo;
...
@RestController
@RequestMapping("/SWZZ_DATA_sys_reportlog")
public class Sys_reportlogController {
    @Autowired
    private Sys_reportlogData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<Sys_reportlogPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody Sys_reportlogPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,Sys_reportlogPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody Sys_reportlogPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,Sys_reportlogPojo.class))){
            watch.stop();
