...
import swzzmodeserver.workserver.data.zjtyphoon.ZJ_TFTHData;
...
@RestController
@RequestMapping("/ZJ_TYPHOON_ZJ_TFTH")
public class ZJ_TFTHController {
    @Autowired
    private ZJ_TFTHData data;
...
//        }
        ...
        List<ZJ_TFTHPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ZJ_TFTHPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ZJ_TFTHPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ZJ_TFTHPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ZJ_TFTHPojo.class))){
            watch.stop();
    ...
    public ResultUtils remove(@RequestBody ParamField bpPojo){
        ...
        Integer num = data.deleteOne(ID,ID2);
        watch.stop();
        if(num > 0){
            return new ResultUtils<>(num, "操作成功",true, num,watch.getTime());
        }else {
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<ZJ_TFTHPojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String type = "";
        if(CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, ZJ_TFTHPojo.class)) || !FieldIsValid.isValid(pattem)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        if(null != pattem){
        ...
        List<ZJ_TFTHPojo> list = new ArrayList<>();
    ...
    public ResultUtils batchRemove(@RequestBody List<ZJ_TFTHPojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, ZJ_TFTHPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        Integer num = 0;
