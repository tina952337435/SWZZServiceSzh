...
import swzzmodeserver.workserver.data.zjtyphoon.ZJ_XSData;
import swzzmodeserver.workserver.data.zjtyphoon.ZJ_XSYBData;
...
@RestController
@RequestMapping("/ZJ_TYPHOON_ZJ_XSYB")
public class ZJ_XSYBController {
    @Autowired
    private ZJ_XSYBData data;
...
//        }
        ...
        List<ZJ_XSYBPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ZJ_XSYBPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ZJ_XSYBPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ZJ_XSYBPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ZJ_XSYBPojo.class))){
            watch.stop();
    ...
    public ResultUtils remove(@RequestBody ParamField bpPojo){
        ...
        if(null != bpPojo.getKID3()){
            ID3 = bpPojo.getKID3();
        }
        Integer num = data.deleteOne(ID1,ID2,ID3);
        watch.stop();
        if(num > 0){
            return new ResultUtils<>(num, "操作成功",true, num,watch.getTime());
        }else {
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<ZJ_XSYBPojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String type = "";
        if(CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, ZJ_XSYBPojo.class)) || !FieldIsValid.isValid(pattem)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        if(null != pattem){
        ...
        List<ZJ_XSYBPojo> list = new ArrayList<>();
    ...
    public ResultUtils batchRemove(@RequestBody List<ZJ_XSYBPojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, ZJ_XSYBPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        Integer num = 0;
