...
import swzzmodeserver.workserver.data.zjtyphoon.ZJ_TFYBLJData;
...
@RestController
@RequestMapping("/ZJ_TYPHOON_ZJ_TFYBLJ")
public class ZJ_TFYBLJController {
    @Autowired
    private ZJ_TFYBLJData data;
...
//        }
        ...
        List<ZJ_TFYBLJPojo> fxList = data.selectList(ID,type,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ZJ_TFYBLJPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ZJ_TFYBLJPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ZJ_TFYBLJPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ZJ_TFYBLJPojo.class))){
            watch.stop();
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<ZJ_TFYBLJPojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String type = "";
        if(CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, ZJ_TFYBLJPojo.class)) || !FieldIsValid.isValid(pattem)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        if(null != pattem){
        ...
        List<ZJ_TFYBLJPojo> list = new ArrayList<>();
