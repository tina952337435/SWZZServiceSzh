...
import swzzmodeserver.workserver.data.swzzflood.ST_TIDEHL_RData;
...
@RestController
@RequestMapping("/SWZZ_FLOOD_ST_TIDEHHL_R")
public class ST_TIDEHHL_RController {
    @Autowired
    private ST_TIDEHHL_RData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<ST_TIDEHHL_RPojo> fxList = data.selectList(IDList,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
        Integer integer = data.selectCount(IDList,key,stime,etime,type);
        Integer count = 1;
        if(!"".equals(pagesize)){
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody ST_TIDEHHL_RPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ST_TIDEHHL_RPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody ST_TIDEHHL_RPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, ST_TIDEHHL_RPojo.class))){
            watch.stop();
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<ST_TIDEHHL_RPojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String type = "";
        if(CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, ST_TIDEHHL_RPojo.class)) || !FieldIsValid.isValid(pattem)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        if(null != pattem){
        ...
        List<ST_TIDEHHL_RPojo> list = new ArrayList<>();
