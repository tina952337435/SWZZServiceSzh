...
import swzzmodeserver.workserver.data.swzzdata.Menu_logData;
...
@RestController
@RequestMapping("/SWZZ_WATER_st_stbprp_b_qu_tree")
public class St_stbprp_b_qu_treeController {
    @Autowired
    private St_stbprp_b_qu_treeData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<St_stbprp_b_qu_treePojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody St_stbprp_b_qu_treePojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        St_stbprp_b_qu_treePojo params = FieldIsValid.getColumnName(bpPojo,St_stbprp_b_qu_treePojo.class);
        if(CommonUtills.isEmpty(params)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody St_stbprp_b_qu_treePojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        St_stbprp_b_qu_treePojo params = FieldIsValid.getColumnName(bpPojo,St_stbprp_b_qu_treePojo.class);
        if(CommonUtills.isEmpty(params)){
