...
@RestController
@RequestMapping("/SWZZ_WATER_st_stbprp_b_qu")
public class St_stbprp_b_quController {
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<St_stbprp_b_quPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    public ResultUtils add(@RequestBody List<St_stbprp_b_quPojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        List<St_stbprp_b_quPojo> params = FieldIsValid.getListColumnName(bpPojo,St_stbprp_b_quPojo.class);
        if(CommonUtills.isEmpty(params)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        Integer num = data.insertList(bpPojo);
        watch.stop();
        if(num > 0){
    ...
    public ResultUtils modify(@RequestBody St_stbprp_b_quPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        St_stbprp_b_quPojo params = FieldIsValid.getColumnName(bpPojo,St_stbprp_b_quPojo.class);
        if(CommonUtills.isEmpty(params)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
