...
public class Tz_ncfilelistController {
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<Tz_ncfilelistPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    public ResultUtils QXSJ_TZ_NCFILELISTSelNew(@RequestBody ParamField bpPojo){
        ...
        List<Map<String, Object>> mapList = service.QXSJ_TZ_NCFILELISTSelNew(startdate, enddate, pattem);
        watch.stop();
        if(mapList.size() > 0){
    ...
    public ResultUtils getshqixiang_yl(@RequestBody ParamField bpPojo){
        ...
        if(bpPojo.getPid().equals(bpPojo.getPid())){
            mapList = mapList.stream().filter(item ->"淀北片小,蕴南片小,嘉宝北片小,青松片小".contains(item.getKEYID())).collect(Collectors.toList());
        }
