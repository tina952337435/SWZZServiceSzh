...
@RestController
@RequestMapping("/SWZZ_QXSJ_st_areatide_ryb")
public class St_areatide_rybController {
    @Autowired
    private St_areatide_rybData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<St_areatide_rybPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody St_areatide_rybPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,St_areatide_rybPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody St_areatide_rybPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo,St_areatide_rybPojo.class))){
            watch.stop();
