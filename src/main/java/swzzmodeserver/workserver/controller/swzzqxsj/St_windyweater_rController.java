...
@RestController
@RequestMapping("/SWZZ_QXSJ_st_windyweater_r")
public class St_windyweater_rController {
    @Autowired
    private St_windyweater_rData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<St_windyweater_rPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody St_windyweater_rPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, St_windyweater_rPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody St_windyweater_rPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, St_windyweater_rPojo.class))){
            watch.stop();
    ...
    public ResultUtils batchResult(@PathVariable("pattem") String pattem, @RequestBody List<St_windyweater_rPojo> bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String type = "";
        if(CommonUtills.isEmpty(FieldIsValid.getListColumnName(bpPojo, St_windyweater_rPojo.class)) || !FieldIsValid.isValid(pattem)){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
        if(null != pattem){
        ...
        List<St_windyweater_rPojo> list = new ArrayList<>();
