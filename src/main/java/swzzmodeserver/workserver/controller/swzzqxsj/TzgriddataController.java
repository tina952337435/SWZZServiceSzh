...
import swzzmodeserver.workserver.data.swzzqxsj.Tz_watershedwgData;
import swzzmodeserver.workserver.data.swzzqxsj.TzgriddataData;
...
@RestController
@RequestMapping("/SWZZ_QXSJ_tzgriddata")
public class TzgriddataController {
    @Autowired
    private TzgriddataData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<TzgriddataPojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody TzgriddataPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, TzgriddataPojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody TzgriddataPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, TzgriddataPojo.class))){
            watch.stop();
    ...
    public ResultUtils remove(@RequestBody TzgriddataPojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        String ID = "",FTM = "",RLSTM = "";
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, TzgriddataPojo.class))){
        ...
        if(null != bpPojo.getGRIDCODE()){
            ID = String.valueOf(bpPojo.getGRIDCODE());
        }
        if(null != bpPojo.getFTM()){
            FTM = bpPojo.getFTM();
        }
        if(null != bpPojo.getRLSTM()){
            RLSTM = bpPojo.getRLSTM();
        }
        Integer num = data.deleteOne(ID,FTM,RLSTM);
        watch.stop();
        if(num > 0){
            return new ResultUtils<>(num, "操作成功",true, num,watch.getTime());
        }else {
