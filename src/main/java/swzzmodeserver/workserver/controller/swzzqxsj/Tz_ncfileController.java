...
import swzzmodeserver.workserver.data.swzzqxsj.Tz_ncfileData;
...
@RestController
@RequestMapping("/SWZZ_QXSJ_tz_ncfile")
public class Tz_ncfileController {
    @Autowired
    private Tz_ncfileData data;
    ...
    public ResultUtils findResult(@RequestBody ParamField bpPojo){
        ...
        List<Tz_ncfilePojo> fxList = data.selectList(ID,key,stime,etime,type,startindex, Integer.valueOf(pagesize));
    ...
    @RequestMapping("/add")
    public ResultUtils add(@RequestBody Tz_ncfilePojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, Tz_ncfilePojo.class))){
            watch.stop();
            return new ResultUtils<>(null,"存在非法字符",false,-1,watch.getTime());
        }
    ...
    public ResultUtils modify(@RequestBody Tz_ncfilePojo bpPojo){
        StopWatch watch = new StopWatch();
        watch.start();
        if(CommonUtills.isEmpty(FieldIsValid.getColumnName(bpPojo, Tz_ncfilePojo.class))){
            watch.stop();
