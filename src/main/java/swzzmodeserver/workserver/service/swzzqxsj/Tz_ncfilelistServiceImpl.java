...
public class Tz_ncfilelistServiceImpl implements Tz_ncfilelistService {
    ...
    public List<Map<String,Object>> QXSJ_TZ_NCFILELISTSelNew(String startdate, String enddate, String pattem) {
        ...
        List<Tz_watersheddataPojo> userList = tzWatersheddataData.selectList("", pattem, startdate, enddate, null, null, null);
        userList.sort(Comparator.comparing(Tz_watersheddataPojo::getFTM));
        List<Map<String,Object>> mapList = new ArrayList<>();
        if (userList.size() > 0){
            List<Tz_watershedPojo> listW = tzWatershedData.selectList("", "", "", "", null, null, null);
            List<String> stcdList = listW.stream().map(Tz_watershedPojo::getKEYID).collect(Collectors.toList());
                ...
                try {
                    for (int num = 0;num < dataCount;num++){
                        ...
                        for (String stcd : stcdList){
                            ...
                            List<Tz_watershedPojo> listBTemp = listW.stream().filter(m -> stcd.equals(m.getKEYID())).collect(Collectors.toList());
                            if (listBTemp.size() > 0){
                ...
                userList.forEach(m->{
                    ...
                    try {
                        if(m.getFTM() != null){
                            ...
                            String strTM = ftm.getMinutes() == 0 ? format.format(ftm) : new SimpleDateFormat("MM月dd日HH时mm分").format(ftm);
                            dto.put("DATESPAN",strTM);
                        }
                    } catch (ParseException e) {
                        e.printStackTrace();
                    }
                    List<Tz_watershedPojo> listBTemp = listW.stream().filter(n -> n.getKEYID().equals(m.getKEYID())).collect(Collectors.toList());
                    if (listBTemp.size() > 0){
                        dto.put("NAME",listBTemp.get(0).getNAME());
                    }
                    mapList.add(dto);
