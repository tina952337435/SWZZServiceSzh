...
public class BDMS_PREDICTServiceImpl implements BDMS_PREDICTService {
    ...
    public String GetFangAnData(String DD_MIND, String STCD, String DATA_TYPE, String DD_EVALUE, String MKEYID) {
        ...
        List<DD_SOLUTIONPojo> listDD = ddSolutionData.selectList("", "", Collections.singletonList(DD_MIND), null, "",
                "", null, null);
            ...
            if (dtTM.size() > 0) {
                ...
                if ("1".equals(DATA_TYPE)) {
                    try {
                        ...
                        // if(rtsqList.size() == 0){
                        // rtsqList =
                        // rtsqData.getSCSWLS(stime,etime,Collections.singletonList(STCD),"");
                        // }
                        // }else {
                        // rtsqList =
                        // rtsqData.getSCSWLS(stime,etime,Collections.singletonList(STCD),"");
                ...
                for (String tm : dtTM) {
                    ...
                    if ("1".equals(DATA_TYPE)) {
                        List<ST_WAS_RPojo> rtsqPojoList = rtsqList.stream()
                                .filter(m -> STCD.trim().equals(m.getSTCD().trim()) && curTM.equals(m.getTM()))
                                .collect(Collectors.toList());
                    ...
                    listDD.forEach(m -> {
                        ...
                        List<ES_ZHANDIANDATAPojo> tempPP = final_pptn_r.stream().filter(
                                s -> s.getSOLUTIONID().trim().equals(dd_id.trim()) && s.getZHANTIME().equals(tm))
                                .collect(Collectors.toList());
                        if (tempPP.size() > 0) {
                            strWhere.put("DRP" + dd_id, tempPP.get(0).getZHANDATA());
                        }
                        List<BDMS_PREDICTPojo> temp = listBDMS.stream()
                                .filter(s -> s.getYMDHM().equals(tm) && s.getPLAN_N().trim().equals(dd_id.trim()))
                                .collect(Collectors.toList());
                        if (temp.size() > 0) {
    ...
    public List<AREASLParam> AREASL(String DD_ID, String STCD) {
        ...
        List<BDMS_PREDICTPojo> bdmsList = bdmsPredictData.selectList("", "", "", Collections.singletonList(DD_ID), null,
                "", null, null, "asc", null);
        ...
        // List<String> stcds =
        // listG.stream().map(ES_MODELGUANLIANPojo::getSTCD).collect(Collectors.toList());
        List<String> mkeyidsCL = listGJY.stream().map(m -> m.getMKEYID().trim()).collect(Collectors.toList());
        List<BDMS_PREDICTPojo> bdmsListRQ = bdmsList.stream().filter(m -> "24".equals(m.getDATA_TYPE()))
                .collect(Collectors.toList());
        List<BDMS_PREDICTPojo> bdmsListCQ = bdmsList.stream()
                .filter(m -> "25".equals(m.getDATA_TYPE()) && mkeyids.contains(m.getSTCD().trim()))
                .collect(Collectors.toList());
        List<BDMS_PREDICTPojo> bdmsListCL = bdmsList.stream()
                .filter(m -> "2".equals(m.getDATA_TYPE()) && mkeyidsCL.contains(m.getSTCD().trim()))
                .collect(Collectors.toList());
        ...
        listG.forEach(m -> {
            ...
            List<BDMS_PREDICTPojo> rq = bdmsListRQ.stream().filter(n -> m.getSTCD().trim().equals(n.getSTCD().trim()))
                    .collect(Collectors.toList());
            List<BDMS_PREDICTPojo> cq = bdmsListCQ.stream().filter(n -> m.getMKEYID().trim().equals(n.getSTCD().trim()))
                    .collect(Collectors.toList());
            ...
            dto.setRSL(String.valueOf(
                    new BigDecimal(Math.round(rsll * 0.36 * 10)).setScale(1, BigDecimal.ROUND_HALF_UP).doubleValue()
                            / 10));
            dto.setCSL(String.valueOf(
                    new BigDecimal(Math.round(csll * 0.36 * 10)).setScale(1, BigDecimal.ROUND_HALF_UP).doubleValue()
                            / 10));
            List<ES_MODELGUANLIANPojo> collect = finalListGJY.stream()
                    .filter(n -> n.getSTCD().trim().equals(m.getSTCD().trim())).collect(Collectors.toList());
            if (collect.size() > 0) {
                double jycsll = bdmsListCL.stream()
                        .filter(n -> n.getSTCD().trim().equals(collect.get(0).getMKEYID().trim()))
                        .mapToDouble(BDMS_PREDICTPojo::getDATA).sum();
                dto.setCLSL(String.valueOf(new BigDecimal(Math.round(jycsll * 0.36 * 10))
                        .setScale(1, BigDecimal.ROUND_HALF_UP).doubleValue() / 10));
    ...
    public List<YBList> YBList(String solutionid, String typeID, String TM) {
        ...
        List<BDMS_PREDICTPojo> listBDMS = bdmsPredictData.selectList("", TM, TM, Collections.singletonList(solutionid),
                null, typeID, null, null, "asc", null);
        List<ES_JISUANZHANPojo> eS_JISUANZHANs = esJisuanzhanData.selectList("", Collections.singletonList(typeID), "",
                null, null);
        ...
        List<swzzmodeserver.workserver.pojo.swzzrtsq.ST_STBPRP_BPojo> dtST_STBPRP_B = rStbprp_BData.selectList(null,
                null);
        ...
        List<ES_ZHANGUANLIANPojo> listGuan = esZhanguanlianData.selectList("", null, null,
                Collections.singletonList("3"));
        if (listBDMS.size() > 0) {
            if (eS_JISUANZHANs.size() > 0) {
                for (ES_JISUANZHANPojo obj : eS_JISUANZHANs) {
                    List<ES_JISUANZHANPXPojo> eS_JISUANZHANsPXTemp = eS_JISUANZHANsPX.stream()
                            .filter(m -> obj.getSTCD().trim().equals(m.getSTCD().trim())).collect(Collectors.toList());
                    String stcd = obj.getSTCD() != null ? obj.getSTCD() : obj.getID();
                    List<BDMS_PREDICTPojo> listBDMSTemp = listBDMS.stream()
                            .filter(m -> stcd.trim().equals(m.getSTCD().trim())
                                    && solutionid.trim().equals(m.getPLAN_N().trim()))
                            .collect(Collectors.toList());
                    List<swzzmodeserver.workserver.pojo.swzzrtsq.ST_STBPRP_BPojo> st_stbrprp_b = dtST_STBPRP_B.stream()
                            .filter(m -> obj.getSTCD().trim().equals(m.getSTCD().trim())).collect(Collectors.toList());
                    String Type = "";
                    List<ES_MODELGUANLIANPojo> listGtemp = listG.stream()
                            .filter(m -> stcd.trim().equals(m.getSTCD().trim())).collect(Collectors.toList());
                    if (listGtemp.size() > 0) {
                        List<ES_SLTONGJIPojo> listSLTemp = listSL.stream()
                                .filter(m -> listGtemp.get(0).getMKEYID().trim().equals(m.getSTCD().trim()))
                                .collect(Collectors.toList());
                    ...
                    String angle = "";
                    List<ES_ZHANGUANLIANPojo> listGuanTemp = listGuan.stream()
                            .filter(m -> stcd.trim().equals(m.getSTCD().trim())).collect(Collectors.toList());
                    ...
                    for (BDMS_PREDICTPojo BDMSobj : listBDMSTemp) {
                        ...
                        List<BDMS_PREDICTPojo> maxOne = listBDMSTemp.stream()
                                .filter(m -> max.equals(m.getDATA().doubleValue())).collect(Collectors.toList());
                        ...
                        if (st_stbrprp_b.size() > 0) {
                            ...
                            List<ST_RVFCCH_BPojo> temp_ST_RVFCCH_B = dtST_RVFCCH_B.stream()
                                    .filter(m -> st_stbrprp_b.get(0).getSTCD().trim().equals(m.getSTCD().trim()))
                                    .collect(Collectors.toList());
    ...
    public List<WJ_MODELSINGRESULTParam> WJ_MODELSINGRESULT(List<String> SOLUTIONID, List<String> STCD,
        ...
        List<DD_SOLUTIONPojo> ListDD = ddSolutionData.selectListByDDID(SOLUTIONID, null, null);

        // List<ST_STBPRP_BDto> ListBasic =
        // stbprpBData.selectListBandStcd("").stream().filter(m ->
        // "1".equals(m.getTYPE().trim())).collect(Collectors.toList());
        ...
        List<ES_JISUANZHANPojo> List_G = esJisuanzhanData.selectList("", Collections.singletonList(DATA_TYPE), "", null,
                null);
        List<ES_JISUANZHANPXPojo> es_jisuanpx = esJisuanzhanpxData.selectList("", null, null).stream()
                .sorted(Comparator.comparingDouble(ES_JISUANZHANPXPojo::getPX)).collect(Collectors.toList());
        ...
        ListDD.forEach(m -> {
            List_G.forEach(n -> {
                ...
                List<BDMS_PREDICTPojo> ListBDMSTemp = finalListBDMS.stream()
                        .filter(s -> s.getPLAN_N().trim().equals(m.getDD_ID().trim())
                                && s.getSTCD().trim().equals(n.getSTCD().trim()))
                        .collect(Collectors.toList());
                ...
                List<swzzmodeserver.workserver.pojo.swzzrtsq.ST_STBPRP_BPojo> List_BT = ListBasic.stream()
                ...
                if (List_BT.size() > 0) {
                    ...
                    dto.setPX(100.0);
                    List<ES_JISUANZHANPXPojo> es_jisuanpxTemp = es_jisuanpx.stream()
                            .filter(s -> s.getSTCD().trim().equals(List_BT.get(0).getSTCD().trim()))
                            .collect(Collectors.toList());
                    if (es_jisuanpxTemp.size() > 0) {
                        dto.setPX(Double.valueOf(es_jisuanpxTemp.get(0).getPX()));
                    }
                    if (ListBDMSTemp.size() > 0) {
                        List<swzzmodeserver.workserver.pojo.swzzrtsq.ST_RVFCCH_BPojo> List_Btemp = List_B.stream()
                                .filter(s -> s.getSTCD().trim().equals(List_BT.get(0).getSTCD().trim()))
                                .collect(Collectors.toList());
                        if (List_Btemp.size() > 0) {
                            if (List_Btemp.get(0).getWRZ() != null && List_Btemp.get(0).getWRZ() != 0) {
                                ...
                                List<BDMS_PREDICTPojo> list = ListBDMSTemp.stream().filter(s -> {
                                    ...
                                    return s.getDATA() >= dto.getWRZ()
                                            && !s.getYMDHM().trim().equals(m.getDD_TM().trim()) && minutes == 0;
                                }).collect(Collectors.toList());
                            ...
                            if (List_Btemp.get(0).getGRZ() != null && List_Btemp.get(0).getGRZ() != 0) {
                                ...
                                List<BDMS_PREDICTPojo> list = ListBDMSTemp.stream()
                                        .filter(s -> s.getDATA() >= dto.getGRZ()).collect(Collectors.toList());
        ...
        System.out.println("WJ_MODELSINGRESULT: 最终结果=" + wjModelsingresultParamList.size());
    ...
    @Override
    public List<Map<String, String>> MODE_BDMS_PREDICTSelMORE(String stime, String etime, List<String> singletonList,
            String stcd, String data_type) {
        List<Map<String, String>> maps = new ArrayList<>();
        List<BDMS_PREDICTPojo> userList = bdmsPredictData
                .selectList("", stime, etime, singletonList, null, data_type, null, null, "asc", null).stream()
                .filter(m -> Arrays.asList(stcd.split(",")).contains(m.getSTCD().trim())).collect(Collectors.toList());
        ...
        userList.forEach(m -> {
            ...
            if (finalSt_rvfcch_b.size() > 0) {
                List<ST_RVFCCH_BPojo> st_rvfcch_bTemp = finalSt_rvfcch_b.stream()
                        .filter(n -> n.getSTCD().trim().equals(m.getSTCD().trim())).collect(Collectors.toList());
            ...
            if (final_WAS_Rs.size() > 0) {
                ...
                List<ST_WAS_RPojo> tempZ = final_WAS_Rs.stream()
                        .filter(n -> n.getTM().equals(ymdhm) && n.getSTCD().trim().equals(m.getSTCD().trim()))
                        .collect(Collectors.toList());
                if (tempZ.size() > 0) {
    ...
    public List<WJ_MODELSINGRESULTHLParam> WJ_MODELSINGRESULTHL(List<String> SOLUTIONID, List<String> STCD,
            String DATA_TYPE) {
